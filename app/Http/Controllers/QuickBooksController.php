...
{
    private ?array $qbSettings = null;

    /** Per-tenant QuickBooks setting (memoized per request) */
    private function qb(string $key)
    {
        $this->qbSettings ??= QuickBooksSetting::values();

        return $this->qbSettings[$key] ?? null;
    }

    private function env(): string
    {
        $e = strtolower(trim((string) $this->qb('env')));
    ...
    {
        return (string) $this->qb('redirect');
    }
    ...
    public function connect(Request $request)
    {
        ...
        $ds = DataService::Configure([
            'auth_mode' => 'oauth2',
            'ClientID' => $this->qb('client_id'),
            'ClientSecret' => $this->qb('client_secret'),
    ...
    /** GET /quickbooks/settings (behind auth) — read from tenant settings */
    public function quickbookgetSettings()
    {
        return response()->json(QuickBooksSetting::values());
    }

    /** POST /quickbooks/settings (behind auth) — write to tenant settings */
    public function saveSettings(Request $request)
    {
        ...
        $setting = QuickBooksSetting::query()->first() ?? new QuickBooksSetting;
        $setting->fill([
            ...
            'income_account_name' => $data['income_account_name'] ?? null,
        ])->save();

        $this->qbSettings = null;
