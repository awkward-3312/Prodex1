...
class SalesController extends BaseController
{
    ...
    public function store(Request $request)
    {
        ...
        $promotionResult = app(PromotionEngine::class)->applyTo([
            'warehouse_id' => (int) $request->warehouse_id,
            'subtotal'     => (float) ($request->input('promotion_subtotal', 0)),
            'item_count'   => (int) ($request->input('promotion_item_count', 0)),
            'product_ids'  => array_map('intval', (array) $request->input('promotion_product_ids', [])),
            'product_subtotals' => (array) $request->input('promotion_product_subtotals', []),
        ...
        $sale = \DB::transaction(function () use ($request, $promotionDiscount, $promotionCodeApplied, $appliedPromotions) {
            ...
            $order->promotion_code = $promotionCodeApplied;
            ...
            // Record one promotion_usages row per applied promotion so caps and
            // reports stay accurate.
            foreach ($appliedPromotions as $applied) {
                PromotionUsage::create([
                    'promotion_id'    => (int) $applied['id'],
                    'sale_id'         => $order->id,
                    'client_id'       => $request->client_id ?: null,
                    'warehouse_id'    => $order->warehouse_id ?: null,
                    'discount_amount' => (float) $applied['amount'],
                    'code'            => $applied['code'] ?? $promotionCodeApplied,
                    'used_at'         => Carbon::now(),
                ]);
            }
        ...
        $qboSync = 'skipped';
        try {
            $realmGuess = $sale->quickbooks_realm_id ?: \App\Models\QuickBooksSetting::values()['realm_id']; // may be null
        ...
        // ZATCA Phase 2: report/clear the invoice (non-blocking; the job is a
        // no-op unless the tenant enabled Phase 2 and finished onboarding).
        try {
    ...
    public function Print_Invoice_POS(Request $request, $id)
    {
        ...
        // promotion_usages records.
        $item['promotions'] = PromotionUsage::with('promotion:id,name,code')
            ->where('sale_id', $sale->id)
            ->orderBy('id', 'asc')
            ->get()
            ->map(function ($u) {
                return [
                    'id'     => (int) $u->promotion_id,
                    'name'   => $u->promotion ? $u->promotion->name : null,
                    'code'   => $u->code,
                    'amount' => number_format((float) $u->discount_amount, helpers::price_decimals(), '.', ''),
                ];
            })
            ->all();
        ...
        // Phase 2: prefer the cryptographically stamped QR (TLV tags 1-9)
        // stored when the invoice was reported/cleared with Fatoora.
        $zatcaQr = null;
        $zatcaDocument = \App\Models\ZatcaDocument::forSource($sale);
        if ($zatcaDocument && ! empty($zatcaDocument->qr_code)) {
            $zatcaQr = $zatcaDocument->qr_code;
        } elseif ($settings && (bool) $settings->zatca_enabled && ! empty($settings->vat_number)) {
    ...
    public function Direct_Network_Print_POS(Request $request, $id)
    {
        ...
        // Per-promotion breakdown (name + amount). Falls back to the single
        // promotion_discount column when no usage rows exist (legacy sales).
        $promoUsages = PromotionUsage::with('promotion:id,name')
            ->where('sale_id', $sale->id)
            ->orderBy('id', 'asc')
            ->get();
        if ($promoUsages->isNotEmpty()) {
            foreach ($promoUsages as $u) {
                $label = 'Promo (' . ($u->promotion ? mb_substr($u->promotion->name, 0, $width - 16) : '#' . $u->promotion_id) . '):';
                $out .= $pad($label, '-' . $symbol . ' ' . number_format((float) $u->discount_amount, helpers::price_decimals(), '.', ''));
            }
        } elseif ((float) ($sale->promotion_discount ?? 0) > 0) {
            $out .= $pad('Promo:', '-' . $symbol . ' ' . number_format((float) $sale->promotion_discount, helpers::price_decimals(), '.', ''));
        }
    ...
    public function Sale_PDF(Request $request, $id)
    {
        ...
        $sale['promotion_discount'] = number_format($sale_data->promotion_discount ?? 0, helpers::price_decimals(), '.', '');
        $sale['promotion_code'] = $sale_data->promotion_code ?? null;
        $sale['promotions'] = PromotionUsage::with('promotion:id,name,code')
            ->where('sale_id', $sale_data->id)
            ->orderBy('id', 'asc')
            ->get()
            ->map(function ($u) {
                return [
    ...
    public function Send_Subscription_Reminder_SMS($subscription_id)
    {
        ...
        try {
            ...
            ErrorLog::create([
                'context' => ucfirst($default_sms_gateway->title).' SMS',
