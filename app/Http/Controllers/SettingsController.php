...
class SettingsController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        Setting::whereId($id)->update([
            ...
            'country_code' => isset($request['country_code']) && $request['country_code'] !== '' && $request['country_code'] !== 'null' ? strtoupper($request['country_code']) : ($setting->country_code ?? 'HN'),
            'tax_regime_code' => isset($request['tax_regime_code']) && $request['tax_regime_code'] !== '' && $request['tax_regime_code'] !== 'null' ? strtoupper($request['tax_regime_code']) : ($setting->tax_regime_code ?? 'SAR'),
            'tax_rate' => isset($request['tax_rate']) && $request['tax_rate'] !== '' && $request['tax_rate'] !== 'null' ? (float) $request['tax_rate'] : ($setting->tax_rate ?? 15.00),
            'locale' => isset($request['locale']) && $request['locale'] !== '' && $request['locale'] !== 'null' ? $request['locale'] : ($setting->locale ?? 'es-HN'),
            'legal_document_label' => isset($request['legal_document_label']) && $request['legal_document_label'] !== '' && $request['legal_document_label'] !== 'null' ? $request['legal_document_label'] : ($setting->legal_document_label ?? 'RTN'),
            'require_rtn' => isset($request['require_rtn']) ? (($request['require_rtn'] == '1' || $request['require_rtn'] == 'true' || $request['require_rtn'] === 1 || $request['require_rtn'] === true) ? 1 : 0) : (int) ($setting->require_rtn ?? 1),
            ...
            // Per-tenant timezone, stored in the tenant DB (never in .env)
            'timezone' => ($request['timezone'] !== null && in_array($request['timezone'], timezone_identifiers_list(), true))
                ? $request['timezone']
            ...
            // Accounting: automatic vs manual journal entry generation
            'accounting_auto_generate_journals' => $request->has('accounting_auto_generate_journals')
                ? (($request['accounting_auto_generate_journals'] == '1' || $request['accounting_auto_generate_journals'] == 'true' || $request['accounting_auto_generate_journals'] === 1 || $request['accounting_auto_generate_journals'] === true) ? 1 : 0)
                : (int) ($setting->accounting_auto_generate_journals ?? 0),
            // Purchase form: extra charges section toggle (default off)
            'purchase_extra_charges_enabled' => $request->has('purchase_extra_charges_enabled')
                ? (($request['purchase_extra_charges_enabled'] == '1' || $request['purchase_extra_charges_enabled'] == 'true' || $request['purchase_extra_charges_enabled'] === 1 || $request['purchase_extra_charges_enabled'] === true) ? 1 : 0)
                : (int) ($setting->purchase_extra_charges_enabled ?? 0),
            // Purchase form: additional custom fields section toggle (default off)
            'purchase_custom_fields_enabled' => $request->has('purchase_custom_fields_enabled')
                ? (($request['purchase_custom_fields_enabled'] == '1' || $request['purchase_custom_fields_enabled'] == 'true' || $request['purchase_custom_fields_enabled'] === 1 || $request['purchase_custom_fields_enabled'] === true) ? 1 : 0)
                : (int) ($setting->purchase_custom_fields_enabled ?? 0),
        ...
        // Apply the new timezone to the current process right away; subsequent
        // requests pick it up via the ApplyTenantTimezone tenancy listener.
        $setting->refresh();
        if ($setting->timezone && in_array($setting->timezone, timezone_identifiers_list(), true)) {
    ...
    public function get_Settings_data_api(Request $request)
    {
        ...
        if ($settings) {
            ...
            // Per-tenant timezone from the tenant DB
            $item['timezone'] = $settings->timezone ?? 'UTC';
            ...
            // Accounting automatic journal generation toggle (default false = manual)
            $item['accounting_auto_generate_journals'] = (bool) ($settings->accounting_auto_generate_journals ?? false);
            // Purchase form section toggles (default off)
            $item['purchase_extra_charges_enabled'] = (bool) ($settings->purchase_extra_charges_enabled ?? false);
            $item['purchase_custom_fields_enabled'] = (bool) ($settings->purchase_custom_fields_enabled ?? false);
            // Cloud backup settings
            ...
            // The loop above mutates the process timezone — restore the tenant's
