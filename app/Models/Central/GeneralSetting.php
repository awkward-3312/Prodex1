...
class GeneralSetting extends Model
{
    ...
    public const LANDING_FONTS = [
        'Inter'=>'Inter','Open Sans'=>'Open Sans','Montserrat'=>'Montserrat','Poppins'=>'Poppins','Nunito'=>'Nunito','Nunito Sans'=>'Nunito Sans','Work Sans'=>'Work Sans','Manrope'=>'Manrope','Sora'=>'Sora','DM Sans'=>'DM Sans','Raleway'=>'Raleway','Mulish'=>'Mulish','Plus Jakarta Sans'=>'Plus Jakarta Sans','Figtree'=>'Figtree','Rubik'=>'Rubik','Karla'=>'Karla','Source Sans 3'=>'Source Sans 3','Playfair Display'=>'Playfair Display','Lora'=>'Lora','Source Serif 4'=>'Source Serif 4',
    ];
    ...
    public function isSharedHosting(): bool { return $this->hosting_mode === 'shared'; }
    public function isVps(): bool { return $this->hosting_mode !== 'shared'; }
    public function remindersEnabled(): bool { return $this->subscription_reminders_enabled ?? true; }
    public function trialRemindersEnabled(): bool { return $this->trial_reminders_enabled ?? true; }
    public function reminderOffsets(): array { return $this->normalizeOffsets($this->subscription_reminder_offsets, self::DEFAULT_REMINDER_OFFSETS); }
    public function trialReminderOffsets(): array { return $this->normalizeOffsets($this->trial_reminder_offsets, self::DEFAULT_TRIAL_REMINDER_OFFSETS); }
    public function reminderChannels(): array { $channels=$this->subscription_reminder_channels; if(!is_array($channels)||empty($channels)) return self::DEFAULT_REMINDER_CHANNELS; $channels=array_values(array_intersect(self::REMINDER_CHANNELS,$channels)); return empty($channels)?self::DEFAULT_REMINDER_CHANNELS:$channels; }
    public function reminderChannelEnabled(string $channel): bool { return in_array($channel, $this->reminderChannels(), true); }
    public function bannerThresholdDays(): int { $value=(int)($this->subscription_banner_threshold_days??0); return $value>0?$value:7; }
    public function reminderSmsTemplate(): string { $template=trim((string)($this->subscription_reminder_sms??'')); return $template!==''?$template:self::DEFAULT_REMINDER_SMS; }
    public function trialSmsTemplate(): string { $template=trim((string)($this->trial_reminder_sms??'')); return $template!==''?$template:self::DEFAULT_TRIAL_SMS; }
    protected function normalizeOffsets($offsets,array $default): array { if(!is_array($offsets)) return $default; $clean=[]; foreach($offsets as $offset){$offset=(int)$offset;if($offset>0)$clean[$offset]=true;} if(empty($clean))return $default; $clean=array_keys($clean);rsort($clean);return $clean; }
    ...
    public static function currencyCode(): string { return static::instance()->currency_code ?? 'USD'; }
    public static function currencySymbol(): string { return static::instance()->currency_symbol ?? '$'; }
    public static function demoDataEnabled(): bool { return (bool) (static::instance()->demo_data_enabled ?? false); }
    public static function whatsappEnabled(): bool { return (bool) (static::instance()->whatsapp_enabled ?? false); }
    public static function resolvedLandingTemplateKey(): string { return LandingPageTemplate::canonicalKey(static::instance()->landing_template); }
     ...
     * Settings form submits only the old single-account fields.
     */
    public function setBankDetailsAttribute($value): void
    {
        ...
        if (! array_key_exists('accounts', $incoming) && isset($current['accounts']) && is_array($current['accounts'])) {
            $incoming['accounts'] = $current['accounts'];
        }

        $this->attributes['bank_details'] = json_encode($incoming, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
    }
    ...
    public function getLogoUrl(): ?string { return $this->logo_path ? asset($this->logo_path) : null; }
    public function getFaviconUrl(): ?string { return $this->favicon_path ? asset($this->favicon_path) : null; }
    public function getTenantLogoUrl(): ?string { return $this->tenant_logo_path ? asset($this->tenant_logo_path) : null; }
    public function getTenantFaviconUrl(): ?string { return $this->tenant_favicon_path ? asset($this->tenant_favicon_path) : null; }
    ...
    public function getReservedSubdomains(): array
    {
        ...
        return array_values(array_unique(array_filter(array_map(static fn ($value) => strtolower(trim((string) $value)), $merged))));
    }
