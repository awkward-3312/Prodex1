...
class QuickBooksService
{
    ...
    /** Per-tenant QuickBooks setting (memoized per instance) */
    private function qbSetting(string $key)
    {
    ...
    private function audit(string $operation, string $level, array $ctx = []): void
    {
        ...
        try {
            QuickBooksAudit::create([
                ...
                'environment' => $this->normalizeEnv($ctx['environment'] ?? $this->qbSetting('env')),
                'operation' => $operation,
    ...
    {
        $env = $this->normalizeEnv($environment ?? $this->qbSetting('env'));
    ...
    private function setupDataService(QuickBooksToken $row): DataService
    {
        ...
        $ds = DataService::Configure([
            'auth_mode' => 'oauth2',
            'ClientID' => $this->qbSetting('client_id'),
            'ClientSecret' => $this->qbSetting('client_secret'),
    ...
    {
        if ($nameCfg = trim((string) $this->qbSetting('income_account_name'))) {
