...
class SyncService
{
     ...
     * - Best-effort downloads the first Woo image into the tenant-aware products upload dir
    ...
    // ---------------------------------------------------------------------
    // MS7-B2-2E — SyncService::syncStock() (POS -> Woo stock push) and its
    // three dedicated computeStockQuantity()/computeVariantStockQuantity()/
    // computeComboStockQuantity() helpers were removed here: confirmed dead
    // (zero callers anywhere in app/) across the whole MS7-B2 series —
    // WooCommerceStockSyncJob (dispatched by
    // WooCommerceSyncController::syncStock()) is the canonical live push
    // path, and reuses ExternalChannelInventoryService::
    // sellableQuantityForFulfillmentWarehouse() (MS7-B2-2C/2C.1) instead.
    // ---------------------------------------------------------------------
    ...
    private function productMainImageSrc(Product $product): ?string
    {
        try {
            ...
            $public = upload_public_path('products').'/'.$imageName;
            if (!is_file($public)) {
                return null;
            }

            return global_asset(upload_path('products').'/'.$imageName);
        } catch (\Throwable $e) {
            return null;
        }
    }
    ...
    private function resolveOrUploadWpMedia(Product $product, ?callable $shouldCancel = null): ?array
    {
        try {
            ...
            $abs = upload_public_path('products').'/'.$imageName;
            if (!is_file($abs)) {
                return null;
            }
