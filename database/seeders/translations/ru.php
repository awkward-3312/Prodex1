<?php

// Language Russian

return [
'Cash_Register_Report' => 'Отчёт по кассе',
'Cashier' => 'Кассир',
'Opening' => 'Начальный баланс',
'Cash In' => 'Приход наличных',
'Cash Out' => 'Расход наличных',
'Opened' => 'Открыто',
'Closed' => 'Закрыто',
'Closing' => 'Конечный баланс',
'Difference' => 'Разница',
'Open Register' => 'Открыть кассу',
'Close Register' => 'Закрыть кассу',
'RegisterOpened' => 'Касса успешно открыта',
'RegisterClosed' => 'Касса успешно закрыта',
'Draft_Created_successfully' => 'Черновик успешно создан',
'Draft_Sale_Deleted' => 'Продажа черновика удалена',
'Inventory_Valuation_Based_on_Average_Cost' => 'Оценка инвентаря на основе средней стоимости',
'Payroll' => 'Заработная плата',
'Transfers_Money' => 'Переводы денег',
'Inventory_Valuation' => 'Оценка инвентаря',
'Deposits_Report' => 'Отчет о депозитах',
'Expense_Report' => 'Отчет о расходах',
'Deposit_Category' => 'Категория депозита',
'Total_Deposits' => 'Всего депозитов',
'Expense_Category' => 'Категория расходов',
'Total_Expenses' => 'Всего расходов',
'ASSET_VALUE' => 'СТОИМОСТЬ АКТИВА',
'STOCK_ON_HAND' => 'Остаток на складе',
'Variant_NAME' => 'Имя варианта',
'SKU' => 'SKU',
'ITEM_NAME' => 'Название товара',
'From_Account' => 'Со счета',
'To_Account' => 'На счет',
'Accounts_cannot_be_the_same' => 'Счета не могут быть одинаковыми',
'CountStock' => 'Подсчет запасов',
'Successfully_Generated_Count' => 'Успешно сгенерированный подсчет',
'Count' => 'Количество',
'file' => 'Файл',
'Accounting' => 'Бухгалтерский учет',
'deposit_delete' => 'Удалить депозит',
'deposit_edit' => 'Редактировать депозит',
'deposit_add' => 'Добавить депозит',
'deposit_view' => 'Просмотреть депозит',
'expense_add' => 'Добавить расход',
'expense_edit' => 'Редактировать расход',
'expense_delete' => 'Удалить расход',
'List_Deposit' => 'Список депозитов',
'Create_deposit' => 'Создать депозит',
'deposit_Category' => 'Категория депозита',
'List_accounts' => 'Список счетов',
'Account' => 'Счет',
'Choose_Account' => 'Выбрать счет',
'Deposits' => 'Депозиты',
'account_num' => 'Номер счета',
'Enter_account_num' => 'Введите номер счета',
'account_name' => 'Название счета',
'Enter_account_name' => 'Введите название счета',
'initial_balance' => 'Начальный баланс',
'Enter_initial_balance' => 'Введите начальный баланс',
'notes' => 'Примечания',
'balance' => 'Баланс',

'Receipt' => 'Чек',
'Pos_Settings' => 'Настройки точки продаж',
'Note_to_customer' => 'Примечание для покупателя',
'Show_Note_to_customer' => 'Показать заметку покупателю',
'Show_barcode' => 'Показать штрих-код',
'Show_Tax_and_Discount' => 'Показать налог, скидки и доставка',
'Show_Customer' => 'Показать клиента',
'Show_Email' => 'Показать электронную почту',
'Show_Phone' => 'Показать телефон',
'Show_Address' => 'Показать адрес',
'DefaultLanguage' => 'Язык по умолчанию',
'footer' => 'нижний колонтитул',
'Received_Amount' => 'Полученная сумма',
'Paying_Amount' => 'Сумма платежа',
'Change' => 'Изменять',
'Paying_amount_is_greater_than_Received_amount' => 'Сумма платежа больше полученной суммы',
'Paying_amount_is_greater_than_Grand_Total' => 'Сумма платежа превышает общую сумму',
'code_must_be_not_exist_already' => 'код уже не должен существовать',
'You_will_find_your_backup_on' => 'Вы найдете резервную копию на',
'and_save_it_to_your_pc' => 'и сохраните на свой компьютер',
'Scan_your_barcode_and_select_the_correct_symbology_below' => 'Отсканируйте свой штрих-код и выберите правильную символику ниже',
'Scan_Search_Product_by_Code_Name' => 'Сканирование / поиск продукта по кодовому названию',
'Paper_size' => 'Размер бумаги',
'Clear_Cache' => 'Очистить кэш',
'Cache_cleared_successfully' => 'Кеш успешно очищен',
'Failed_to_clear_cache' => 'Не удалось очистить кеш',
'Scan_Barcode' => 'Сканер штрих-кода',
'Please_use_short_name_of_unit' => 'Пожалуйста, используйте короткое название единицы',
'DefaultCustomer' => 'Клиент по умолчанию',
'DefaultWarehouse' => 'Склад по умолчанию',
'Payment_Gateway' => 'Платежный шлюз',
'SMS_Configuration' => 'Конфигурация SMS',
'Gateway' => 'Платежный шлюз',
'Choose_Gateway' => 'Выберите платежный шлюз',
'sms_send_successfully' => 'Сообщение успешно отправлено',
'sms_config_invalid' => 'неверная конфигурация смс недействительна',
'Remove_Stripe_Key_Secret' => 'Удалить ключи API Stripe',
'credit_card_account_not_available' => 'Счет кредитной карты недоступен',
'Credit_Card_Info' => 'Информация о кредитной карте',
'developed_by' => 'Разработан',
'Unit_already_linked_with_sub_unit' => 'Объект уже связан с дополнительным блоком',
'Total_Items_Quantity' => 'Итого и количество',
'Value_by_Cost_and_Price' => 'Соотношение затрат и цен',
'Search_this_table' => 'Искать в этой таблице',
'import_products' => 'Импортные товары',
'Field_optional' => 'Поле необязательно',
'Download_exemple' => 'Скачать пример',
'field_must_be_in_csv_format' => 'Поле должно быть в формате csv.',
'Successfully_Imported' => 'Успешно импортировано',
'file_size_must_be_less_than_1_mega' => 'Размер файла не должен превышать 1 мегабайт.',
'Please_follow_the_import_instructions' => 'Следуйте инструкциям по импорту',
'must_be_exist' => 'юнит уже должен быть создан',
'Import_Customers' => 'Импортные клиенты',
'Import_Suppliers' => 'Поставщики импорта',
'Recent_Sales' => 'Последние продажи',
'Create_Transfer' => 'Создать перевод',
'order_products' => 'элементы заказа',
'Search_Product_by_Code_Name' => 'Поиск продукта по коду или названию',
'Reports_payments_Purchase_Return' => 'Отчеты о возвратных платежах за покупку',
'Reports_payments_Sale_Return' => 'Отчеты о продажах и возвратах платежей',
'payments_Sales_Return' => 'возврат платежей',
'payments_Purchases_Return' => 'платежи покупки возврат',
'CreateSaleReturn' => 'Создать возврат продажи',
'EditSaleReturn' => 'Изменить возврат продажи',
'SalesReturn' => 'Возвращение продаж',
'CreatePurchaseReturn' => 'Создать возврат покупки',
'EditPurchaseReturn' => 'Изменить возврат покупки',
'PurchasesReturn' => 'Покупки Возврат',
'Due' => 'должный',
'Profit' => 'Выгода',
'Revenue' => 'Доход',
'Sales_today' => 'Продажи сегодня',
'People' => 'Люди',
'Successfully_Created' => 'Успешно создано',
'Successfully_Updated' => 'Успешно обновлено',
'Success' => 'Успех',
'Failed' => 'Не удалось',
'Warning' => 'Warning',
'Please_fill_the_form_correctly' => 'Пожалуйста, заполните форму правильно',
'Field_is_required' => 'Поле, обязательное для заполнения',
'Error' => 'Ошибка!',
'you_are_not_authorized' => 'Извини! вы не авторизованы.',
'Go_back_to_home' => 'Вернуться на главную',
'page_not_exist' => 'Извини! Страница, которую вы искали, не существует.',
'Choose_Status' => 'Выберите статус',
'Choose_Method' => 'Выберите метод',
'Choose_Symbology' => 'Выберите символы',
'Choose_Category' => 'Выберите категорию',
'Choose_Customer' => 'Выберите клиента',
'Choose_Supplier' => 'Выберите поставщика',
'Choose_Unit_Purchase' => 'Выберите покупную единицу',
'Choose_Sub_Category' => 'Выберите подкатегорию',
'Choose_Brand' => 'Выберите марку',
'Choose_Warehouse' => 'Выберите склад',
'Choose_Unit_Sale' => 'Выберите единицу продажи',
'Enter_Product_Cost' => 'Введите стоимость продукта',
'Enter_Stock_alert' => 'Введите оповещение о наличии акций',
'Choose_Unit_Product' => 'Выберите единицу продукции',
'Enter_Product_Price' => 'Введите цену продукта',
'Enter_Name_Product' => 'Введите название продукта',
'Enter_Role_Name' => 'Введите имя роли',
'Enter_Role_Description' => 'Введите описание роли',
'Enter_name_category' => 'Введите название категории',
'Enter_Code_category' => 'Введите код категории',
'Enter_Name_Brand' => 'Введите название бренда',
'Enter_Description_Brand' => 'Введите описание бренда',
'Enter_Code_Currency' => 'Введите валюту кода',
'Enter_name_Currency' => 'Введите имя Валюта',
'Enter_Symbol_Currency' => 'Введите валюту символа',
'Enter_Name_Unit' => 'Введите название объекта',
'Enter_ShortName_Unit' => 'Введите короткое имя',
'Choose_Base_Unit' => 'Выберите базовый блок',
'Choose_Operator' => 'Выберите оператора',
'Enter_Operation_Value' => 'Введите значение операции',
'Enter_Name_Warehouse' => 'Введите название склада',
'Enter_Phone_Warehouse' => 'Введите телефон склада',
'Enter_Country_Warehouse' => 'Укажите страну склада',
'Enter_City_Warehouse' => 'Введите город склада',
'Enter_Email_Warehouse' => 'Введите адрес электронной почты склада',
'Enter_ZipCode_Warehouse' => 'Введите почтовый индекс склада',
'Choose_Currency' => 'Выберите Валюту',
'Thank_you_for_your_business' => 'Спасибо за ваш бизнес!',
'Cancel' => 'Отмена',
'New_Customer' => 'Новый покупатель',
'Incorrect_Login' => 'Неправильный логин',
'Successfully_Logged_In' => 'Успешный вход в систему',
'This_user_not_active' => 'Этот пользователь не активен',
'SignIn' => 'Войти',
'Create_an_account' => 'Завести аккаунт',
'Forgot_Password' => 'Забыл пароль ?',
'Email_Address' => 'Адрес электронной почты',
'SignUp' => 'Зарегистрироваться',
'Already_have_an_account' => 'Уже есть аккаунт?',
'Reset_Password' => 'Сброс пароля',
'Failed_to_authenticate_on_SMTP_server' => 'Не удалось пройти аутентификацию на SMTP-сервере',
'We_cant_find_a_user_with_that_email_addres' => 'Мы не можем найти пользователя с таким адресом электронной почты',
'We_have_emailed_your_password_reset_link' => 'Мы отправили вам ссылку для сброса пароля по электронной почте',
'Please_fill_the_Email_Adress' => 'Пожалуйста, заполните адрес электронной почты',
'Confirm_password' => 'Подтвердить Пароль',
'Your_Password_has_been_changed' => 'Ваш пароль был изменен',
'The_password_confirmation_does_not_match' => 'Подтверждение пароля не совпадает',
'This_password_reset_token_is_invalid' => 'Этот токен сброса пароля недействителен',
'Warehouse_report' => 'Отчет о складе',
'All_Warehouses' => 'Все склады',
'Expense_List' => 'Список расходов',
'Expenses' => 'Затраты',
'This_Week_Sales_Purchases' => 'Продажи и покупки на этой неделе',
'Top_Selling_Products' => 'Самые продаваемые товары',
'View_all' => 'Посмотреть все',
'Payment_Sent_Received' => 'Платеж отправлен и получен',
'Filter' => 'Фильтр',
'Invoice_POS' => 'Счет-фактура POS',
'Invoice' => 'Выставленный счет',
'Customer_Info' => 'Информация о клиенте',
'Company_Info' => 'информация о компании',
'Invoice_Info' => 'Информация о счете',
'Order_Summary' => 'итог заказа',
'Quote_Info' => 'Ценовая информация',
'Del' => 'удалять',
'SuppliersPaiementsReport' => 'Отчет о платежах поставщикам',
'Purchase_Info' => 'Информация о покупке',
'Supplier_Info' => 'Информация о поставщиках',
'Return_Info' => 'информация о возврате',
'Expense_Category' => 'Категория расходов',
'Create_Expense' => 'Создать расход',
'Details' => 'Детали',
'Discount_Method' => 'Метод скидки',
'Net_Unit_Cost' => 'Чистая стоимость единицы',
'Net_Unit_Price' => 'Чистая цена за единицу',
'Edit_Expense' => 'Изменить расходы',
'All_Brand' => 'Все марки',
'All_Category' => 'Все категории',
'ListExpenses' => 'Перечислить расходы',
'Create_Permission' => 'Создать разрешение',
'Edit_Permission' => 'Изменить разрешение',
'Reports_payments_Sales' => 'Отчеты о платежах Продажи',
'Reports_payments_Purchases' => 'Отчеты о платежах Покупки',
'Reports_payments_Return_Customers' => 'Отчеты о платежах Возврат клиентов',
'Reports_payments_Return_Suppliers' => 'Отчеты о платежах Возврат Поставщикам',
'Expense_Deleted' => 'Этот расход был удален',
'Expense_Updated' => 'Этот расход был обновлен',
'Expense_Created' => 'Этот расход был создан',
'DemoVersion' => 'Вы не можете этого сделать в демо-версии',
'OrderStatistics' => 'Статистика продаж',
'AlreadyAdd' => 'Этот продукт уже добавлен!',
'AddProductToList' => 'Пожалуйста, добавьте товар в список!',
'AddQuantity' => 'Пожалуйста, добавьте количество деталей !!',
'InvalidData' => 'Неверные данные !!',
'LowStock' => 'количество превышает количество на складе',
'WarehouseIdentical' => 'Два склада не могут быть идентичными !!',
'VariantDuplicate' => 'Этот вариант дублируется !!',
'Filesize' => 'Размер файла',
'GenerateBackup' => 'Создать резервную копию',
'BackupDatabase' => 'Резервная база данных',
'Backup' => 'резервное копирование',
'Paid' => 'Платный',
'Unpaid' => 'Неоплаченный',
'Today' => 'Cегодня',
'Income' => 'Доход',
'Expenses' => 'Затраты',
'Sale' => 'распродажа',
'Actif' => 'Активный',
'Inactif' => 'Неактивный',
'Customers' => 'Клиенты',
'Phone' => 'Телефон',
'SearchByPhone' => 'Поиск по телефону',
'Suppliers' => 'Поставщики',
'Quotations' => 'Котировки',
'Sales' => 'Продажи',
'Purchases' => 'Покупки',
'Returns' => 'Возврат',
'Settings' => 'Настройки',
'SystemSettings' => 'Системные настройки',
'Users' => 'Пользователи',
'GroupPermissions' => 'Групповые разрешения',
'Currencies' => 'Валюты',
'Warehouses' => 'Склады',
'Units' => 'Единицы',
'UnitsPrchases' => 'Покупка единиц',
'UnitsSales' => 'Единицы продаж',
'Reports' => 'Отчеты',
'PaymentsReport' => 'Отчет о платежах',
'PaymentsPurchases' => 'Платежи Покупки',
'PaymentsSales' => 'Платежи Продажи',
'ProfitandLoss' => 'Прибыль и убыток',
'WarehouseStockChart' => 'График складских запасов',
'SalesReport' => 'Отчет о продажах',
'PurchasesReport' => 'Отчет о закупках',
'CustomersReport' => 'Отчет клиентов',
'SuppliersReport' => 'Отчет о поставщиках',
'SupplierReport' => 'Отчет поставщика',
'DailySalesData' => 'Ежедневные данные о продажах',
'DailyPurchasesData' => 'Данные о ежедневных покупках',
'Dernièrescinqrecords' => 'Последние пять рекордов',
'Filters' => 'Фильтры',
'date' => 'Дата',
'Reference' => 'Справка',
'Supplier' => 'Поставщик',
'PaymentStatus' => 'Статус платежа',
'Customer' => 'Покупатель',
'CustomerCode' => 'Код клиента',
'Status' => 'Положение дел',
'SupplierCode' => 'Код поставщиком',
'Categorie' => 'Категория',
'Categories' => 'Категории',
'StockTransfers' => 'Передача запасов',
'StockManagement' => 'Управление запасами',
'dashboard' => 'Щиток приборов',
'Products' => 'Товары',
'productsList' => 'Список продуктов',
'ProductManagement' => 'Управление продуктом',
'ProductQuantityAlerts' => 'Оповещения о количестве товаров',
'CodeProduct' => 'Код продукта',
'ProductTax' => 'Налог на продукцию',
'SubCategorie' => 'Подкатегория',
'Name_product' => 'Обозначение',
'StockAlert' => 'Уведомление о наличии запасов',
'warehouse' => 'склад',
'Tax' => 'Налог',
'BuyingPrice' => 'Цена покупки',
'SellPrice' => 'Цена продажи',
'Quantity' => 'Количество',
'UnitSale' => 'Продажа единиц',
'UnitPurchase' => 'Покупка единицы',
'ManagementCurrencies' => 'Валютный менеджмент',
'CurrencyCode' => 'Код валюты',
'CurrencyName' => 'Название валюты',
'Symbol' => 'Символ',
'All' => 'Все',
'EditProduct' => 'Редактировать продукт',
'SearchByCode' => 'Поиск по коду',
'SearchByName' => 'Поиск по имени',
'ProductDetails' => 'информация о продукте',
'CustomerName' => 'Имя Клиента',
'CustomerManagement' => 'Управление клиентами',
'Add' => 'Создайте',
'add' => 'Создайте',
'Edit' => 'редактировать',
'Close' => 'близко',
'PleaseSelect' => 'Пожалуйста выберите',
'Action' => 'Действие',
'Email' => 'Электронное письмо',
'EditCustomer' => 'Изменить клиента',
'AddCustomer' => 'Создать клиента',
'Country' => 'Страна',
'City' => 'город',
'Adress' => 'Адрес',
'CustomerDetails' => 'Детали клиента',
'CustomersList' => 'Список клиентов',
'SupplierCode' => 'Код поставщиком',
'SupplierName' => 'наименование поставщика',
'SuppliersManagement' => 'Управление поставщиками',
'SupplierDetails' => 'Детали поставщика',
'QuotationsManagement' => 'Управление котировками',
'SubTotal' => 'Промежуточный итог',
'MontantReste' => 'Осталась сумма',
'complete' => 'завершено',
'EnAttendant' => 'в ожидании',
'Recu' => 'Получила',
'partial' => 'Частичное',
'Retournee' => 'Возвращение',
'DetailQuote' => 'Подробная цитата',
'EditQuote' => 'Изменить цитату',
'CreateSale' => 'Создать распродажу',
'DownloadPdf' => 'Скачать Pdf',
'QuoteEmail' => 'Предложение отправлено по электронной почте',
'DeleteQuote' => 'Удалить предложение',
'AddQuote' => 'Создать предложение',
'SelectProduct' => 'Выбрать продукт',
'ProductCodeName' => 'Продукт (Код - Название)',
'Price' => 'Цена',
'CurrentStock' => 'Склад',
'Total' => 'Общее',
'Num' => '№',
'Unitcost' => 'стоимость единицы',
'to' => 'к',
'Subject' => 'Предмет',
'Message' => 'Сообщение',
'EmailCustomer' => 'Электронная почта клиента',
'Sent' => 'послать',
'Quote' => 'Цитата',
'Hello' => 'Здравствуйте',
'AttachmentQuote' => 'Пожалуйста, найдите приложение к вашему предложению',
'AddProducts' => 'Добавить товары в список заказов',
'SelectWarehouse' => 'Пожалуйста, выберите склад',
'SelectCustomer' => 'пожалуйста, выберите клиента',
'SalesManagement' => 'Управление продажами',
'Balance' => 'Остаток средств',
'QtyBack' => 'Назад Количество',
'TotalReturn' => 'Общий доход',
'Amount' => 'Количество',
'SaleDetail' => 'Детали продажи',
'EditSale' => 'Изменить распродажу',
'AddSale' => 'Создать распродажу',
'ShowPayment' => 'Показать платежи',
'AddPayment' => 'Создать платеж',
'EditPayment' => 'Изменить платеж',
'EmailSale' => 'Отправить распродажу по электронной почте',
'DeleteSale' => 'Удалить распродажу',
'ModePaiement' => 'Оплачивается',
'Paymentchoice' => 'Выбор платежа',
'Note' => 'Заметка',
'PaymentComplete' => 'Оплата завершена!',
'PurchasesManagement' => 'Управление закупками',
'Ordered' => 'Упорядоченный',
'DeletePurchase' => 'Удалить покупку',
'EmailPurchase' => 'Отправить покупку по электронной почте',
'EditPurchase' => 'Изменить покупку',
'PurchaseDetail' => 'Детали покупки',
'AddPurchase' => 'Создать покупку',
'EmailSupplier' => 'Электронная почта поставщика',
'PurchaseInvoice' => 'Оплата покупок',
'PurchasesInvoicesData' => 'Данные о платежах покупок',
'SalesInvoice' => 'Платежи за продажу',
'SalesInvoicesData' => 'Данные о продажах',
'UserManagement' => 'управление пользователями',
'Firstname' => 'Имя',
'lastname' => 'фамилия',
'username' => 'Имя пользователя',
'password' => 'пароль',
'Newpassword' => 'Новый пароль',
'ChangeAvatar' => 'Сменить аватар',
'LeaveBlank' => 'Оставьте это поле пустым, если вы его не меняли',
'type' => 'тип',
'UserPermissions' => 'Разрешения пользователей',
'RoleName' => 'Роль',
'RoleDescription' => 'Описание роли',
'AddPermissions' => 'Создать разрешения',
'View' => 'Посмотреть',
'Del' => 'удалять',
'NewAdjustement' => 'Новая корректировка',
'EditAdjustement' => 'Изменить настройку',
'CannotSubstraction' => 'Вы не можете вычесть продукты, у которых есть запас 0',
'Addition' => 'Дополнение',
'Subtraction' => 'Вычитание',
'profil' => 'профиль',
'logout' => 'выйти',
'PurchaseAlreadyPaid' => 'вы не можете изменить, потому что эта покупка уже оплачена',
'SaleAlreadyPaid' => 'вы не можете изменить, потому что эта Распродажа уже оплачена',
'ReturnAlreadyPaid' => 'вы не можете изменить, потому что этот возврат уже оплачен',
'QuoteAlready' => 'Эта цитата уже привела к продаже',
'AddProduct' => 'Создать продукт',
'QuotationComplete' => 'Цитата завершена',
'SiteConfiguration' => 'Конфигурация сайта',
'Language' => 'Язык',
'DefaultCurrency' => 'Валюта по умолчанию',
'LoginCaptcha' => 'Войти Captcha',
'DefaultEmail' => 'Почта по умолчанию',
'SiteName' => 'Название сайта',
'ChangeLogo' => 'Изменить логотип',
'SMTPConfiguration' => 'Конфигурация SMTP',
'HOST' => 'ВЕДУЩИЙ',
'PORT' => 'ПОРТ',
'encryption' => 'Шифрование',
'SMTPIncorrect' => 'Неправильная конфигурация SMTP',
'PaymentsReturns' => 'Платежи Возврат',
'ReturnsInvoices' => 'Возврат счетов',
'ReturnsInvoicesData' => 'Возвращает данные счетов-фактур',
'ShowAll' => 'Показать все записи всех пользователей',
'Discount' => 'Скидка',
'OrderTax' => 'Налог на заказ',
'Shipping' => 'Перевозка',
'CompanyName' => 'Название компании',
'CompanyPhone' => 'Телефон компании',
'CompanyAdress' => 'Адрес компании',
'Code' => 'Код',
'image' => 'образ',
'Printbarcode' => 'Распечатать штрих-код',
'ReturnsCustomers' => 'Возвращает клиентов',
'ReturnsSuppliers' => 'Возврат Поставщикам',
'FactureReturnCustomers' => 'Возврат счета-фактуры клиента',
'FactureReturnSuppliers' => 'Счет-фактура возврата поставщика',
'NodataAvailable' => 'Данные недоступны',
'ProductImage' => 'Изображение продукта',
'Barcode' => 'Штрих-код',
'pointofsales' => 'Точка продаж',
'CustomUpload' => 'Пользовательская загрузка',
'pointofsaleManagement' => 'точка управления продажами',
'Adjustment' => 'Регулировка',
'Updat' => 'Обновить',
'Reset' => 'Сброс настроек',
'print' => 'Распечатать',
'SearchByEmail' => 'Поиск по почте',
'ChooseProduct' => 'Выберите продукт',
'Qty' => 'Кол-во',
'Items' => 'Предметы',
'AmountHT' => 'Количество',
'AmountTTC' => 'общая сумма',
'PleaseSelectSupplier' => 'Пожалуйста, выберите поставщика',
'PleaseSelectStatut' => 'Пожалуйста, выберите статус',
'PayeBy' => 'Оплачивается',
'ChooseWarehouse' => 'Выберите склад',
'payNow' => 'Заплатить сейчас',
'ListofCategory' => 'Список категорий',
'Description' => 'Описание',
'submit' => 'представить',
'ProblemCreatingThisInvoice' => 'При создании этого счета возникла проблема. Пожалуйста, попробуйте еще раз',
'ProblemPayment' => 'Возникла проблема с оплатой. Пожалуйста, попробуйте еще раз.',
'IncomeExpenses' => 'Доходы и расходы',
'dailySalesPurchases' => 'Ежедневные продажи и покупки',
'ProductsExpired' => 'Срок действия продуктов истек',
'ListofBrand' => 'Список брендов',
'CreateAdjustment' => 'Создать корректировку',
'Afewwords' => 'Несколько слов ...',
'UserImage' => 'Изображение пользователя',
'UpdateProduct' => 'Обновить продукт',
'Brand' => 'Марка',
'BarcodeSymbology' => 'Символика штрих-кода',
'ProductCost' => 'Стоимость продукта',
'ProductPrice' => 'Цена продукта',
'UnitProduct' => 'Единица продукта',
'TaxMethod' => 'Налоговый метод',
'MultipleImage' => 'Несколько изображений',
'ProductHasMultiVariants' => 'Продукт имеет несколько вариантов',
'ProductHasPromotion' => 'У продукта есть продвижение',
'PromotionStart' => 'Начало продвижения',
'PromotionEnd' => 'Конец акции',
'PromotionPrice' => 'Цена акции',
'Price' => 'Цена',
'Cost' => 'Стоимость',
'Unit' => 'Ед. изм',
'ProductVariant' => 'Вариант продукта',
'Variant' => 'Вариант',
'UnitPrice' => 'Цена за единицу',
'CreateReturnCustomer' => 'Создать возвратного клиента',
'EditReturnCustomer' => 'Изменить возвратного клиента',
'CreateReturnSupplier' => 'Создать поставщика возврата',
'Documentation' => 'Документация',
'EditReturnSupplier' => 'Изменить поставщика возврата',
'FromWarehouse' => 'Со склада',
'ToWarehouse' => 'На склад',
'EditTransfer' => 'Редактировать перевод',
'TransferDetail' => 'Детали перевода',
'Pending' => 'В ожидании',
'Received' => 'Получила',
'Ordered' => 'Упорядоченный',
'PermissionsManager' => 'Управление разрешениями',
'BrandManager' => 'Марка',
'BrandImage' => 'Фирменное изображение',
'BrandName' => 'Имя бренда',
'BrandDescription' => 'Описание бренда',
'BaseUnit' => 'Базовый блок',
'ManagerUnits' => 'Управление подразделениями',
'OperationValue' => 'Значение операции',
'Operator' => 'Оператор',
'Top5Products' => 'Пять лучших продуктов',
'Last5Sales' => 'Последние пять продаж',
'ListAdjustments' => 'Список корректировок',
'ListTransfers' => 'Список переводов',
'CreateTransfer' => 'Создать перевод',
'OrdersManager' => 'Управление заказами',
'ListQuotations' => 'Список котировок',
'ListPurchases' => 'Список покупок',
'ListSales' => 'Список продаж',
'ListReturns' => 'Список возвратов',
'PeopleManager' => 'Управление персоналом',
'Delete_Title' => 'Ты уверен?',
'Delete_Text' => 'Вы не сможете отменить это!',
'Delete_confirmButtonText' => 'Да удалите!',
'Delete_cancelButtonText' => 'Отмена',
'Delete_Deleted' => 'Удалено!',
'Delete_Failed' => 'Не удалось!',
'Delete_Therewassomethingwronge' => 'Что-то было не так',
'SendEmail' => 'Электронная почта отправлена успешно',
'erroSale' => 'эта распродажа уже связана с возвратом!',
'ReturnManagement' => 'Управление возвратом',
'ReturnDetail' => 'Деталь возврата',
'EditReturn' => 'Изменить возврат',
'AddReturn' => 'Создать возврат',
'EmailReturn' => 'Отправить возврат по почте',
'DeleteReturn' => 'Удалить возврат',
'Retoursurcharge' => 'Return Surcharge',
'Laivrison' => 'Доставка',
'SelectSale' => 'Выбрать распродажу',
'ZeroPardefault' => 'Вы можете удалить товар или установить возвращаемое количество равным нулю, если оно не возвращается.',
'Return' => 'Возвращение',
'Purchase' => 'Покупка',
'TotalSales' => 'Тотальная распродажа',
'TotalPurchases' => 'Всего покупок',
'TotalReturns' => 'Общая прибыль',
'PaiementsNet' => 'Чистые платежи',
'PaiementsSent' => 'Отправленные платежи',
'PaiementsReceived' => 'Платежи получены',
'Recieved' => 'Получила',
'Sent' => 'Отправлено',
'ProductQuantityAlerts' => 'Оповещения о количестве товаров',
'ProductCode' => 'Код',
'ProductName' => 'Товар',
'AlertQuantity' => 'Количество предупреждений',
'WarehouseStockChart' => 'График складских запасов',
'TotalProducts' => 'Всего продуктов',
'TotalQuantity' => 'Общая численность',
'TopCustomers' => 'Пятерка крупнейших клиентов',
'TotalAmount' => 'Итого',
'TotalPaid' => 'Общая сумма',
'CustomerSalesReport' => 'Отчет о продажах клиентов',
'CustomerPaiementsReport' => 'Отчет о платежах клиентов',
'CustomerQuotationsReport' => 'Отчет о предложениях клиентов',
'Payments' => 'Платежи',
'TopSuppliers' => 'Пятерка лучших поставщиков',
'SupplierPurchasesReport' => 'Отчет о закупках поставщика',
'SupplierPaiementsReport' => 'Отчет о платежах поставщикам',
'Name' => 'название',
'Code' => 'Код',
'ManagementWarehouse' => 'Управление складом',
'ZipCode' => 'Почтовый Индекс',
'managementCategories' => 'Управление категориями',
'Codecategorie' => 'Категория кода',
'Visible_in_online_store' => 'Виден в интернет-магазине',
'Namecategorie' => 'Категория имени',
'Parentcategorie' => 'Родительская категория',
'managementTax' => 'Налоговый менеджмент',
'TaxName' => 'Название налога',
'TaxRate' => 'Ставка налога',
'managementUnitPurchases' => 'Единица закупок',
'managementUnitSales' => 'Отдел продаж',
'ShortName' => 'Короткое имя',
'PleaseSelectThesebeforeaddinganyproduct' => 'Пожалуйста, выберите их перед добавлением любого продукта',
'StockAdjustement' => 'Регулировка запаса',
'PleaseSelectWarehouse' => 'Пожалуйста, выберите склад, прежде чем выбрать любой продукт',
'StockTransfer' => 'Передача запасов',
'SelectPeriod' => 'Выберите период',
'ThisYear' => 'В этом году',
'ThisToday' => 'Это сегодня',
'ThisMonth' => 'Этот месяц',
'ThisWeek' => 'Эта неделя',
'AdjustmentDetail' => 'Детали настройки',
'ActivateUser' => 'Этот пользователь был активирован',
'DisActivateUser' => 'Этот пользователь был деактивирован',
'NotFound' => 'Страница не найдена.',
'oops' => 'ошибка! Страница не найдена.',
'couldNotFind' => 'Мы не смогли найти страницу, которую вы искали. Тем временем вы можете',
'ReturnDashboard' => 'вернуться на панель управления',
'hrm' => 'УЧР',
'Employees' => 'Сотрудники',
'Attendance' => 'Посещаемость',
'Leave_request' => 'Оставьте запрос',
'Leave_type' => 'Оставить Тип',
'Company' => 'Компания',
'Departments' => 'Департаменты',
'Designations' => 'Обозначения',
'Office_Shift' => 'Офисная смена',
'Holidays' => 'Праздники',
'Enter_Company_Name' => 'Введите название компании',
'Enter_email_address' => 'Введите адрес электронной почты',
'Enter_Company_Phone' => 'Введите телефон компании',
'Enter_Company_Country' => 'Введите страну компании',
'Created_in_successfully' => 'Создано успешно',
'Updated_in_successfully' => 'Обновлено успешно',
'Deleted_in_successfully' => 'Удален в успешно',
'department' => 'отделение',
'Enter_Department_Name' => 'Введите название отдела',
'Choose_Company' => 'Выберите компанию',
'Department_Head' => 'Начальник отдела',
'Choose_Department_Head' => 'Выбрать начальника отдела',
'Enter_Shift_name' => 'Введите название смены',
'Monday_In' => 'Monday In',
'Monday_Out' => 'Monday Out',
'Tuesday_In' => 'Tuesday In',
'tuesday_out' => 'tuesday Out',
'wednesday_in' => 'Wednesday In',
'wednesday_out' => 'Wednesday Out',
'thursday_in' => 'Thursday In',
'thursday_out' => 'Thursday Out',
'friday_in' => 'Friday In',
'friday_out' => 'Friday Out',
'saturday_in' => 'Saturday In',
'saturday_out' => 'Saturday Out',
'sunday_in' => 'Sunday In',
'sunday_out' => 'Sunday Out',
'Holiday' => 'Праздничный день',
'Enter_title' => 'Введите название',
'title' => 'заглавие',
'start_date' => 'Дата начала',
'Enter_Start_date' => 'Введите дату начала',
'Finish_Date' => 'Дата окончания',
'Enter_Finish_date' => 'Введите дату окончания',
'Please_provide_any_details' => 'Пожалуйста, предоставьте любую информацию',
'Attendances' => 'Посещаемость',
'Enter_Attendance_date' => 'Введите дату присутствия',
'Time_In' => 'Time In',
'Time_Out' => 'Time Out',
'Choose_Employee' => 'Выберите сотрудника',
'Employee' => 'Сотрудник',
'Work_Duration' => 'Длительность работы',
'remaining_leaves_are_insufficient' => 'Остальные листья недостаточны',
'Leave_Type' => 'Оставить Тип',
'Days' => 'Дней',
'Department' => 'отделение',
'Choose_leave_type' => 'Выберите тип отпуска',
'Choose_status' => 'Выберите статус',
'Leave_Reason' => 'Оставить причину',
'Enter_Reason_Leave' => 'Укажите причину Оставить',
'Add_Employee' => 'Добавить сотрудника',
'FirstName' => 'имя',
'Enter_FirstName' => 'Введите имя',
'LastName' => 'Фамилия',
'Enter_LastName' => 'Введите фамилию',
'Gender' => 'гендер',
'Choose_Gender' => 'Выберите пол',
'Enter_Birth_date' => 'Введите дату рождения',
'Birth_date' => 'Дата рождения',
'Enter_Country' => 'Введите страну',
'Enter_Phone_Number' => 'Введите номер телефона',
'joining_date' => 'Дата вступления',
'Enter_joining_date' => 'Введите дату присоединения',
'Choose_Designation' => 'Выберите обозначение',
'Designation' => 'Обозначение',
'Office_Shift' => 'Офисная смена',
'Choose_Office_Shift' => 'Выбрать офисную смену',
'Enter_Leaving_Date' => 'Введите дату отъезда',
'Leaving_Date' => 'Дата отъезда',
'Annual_Leave' => 'Ежегодный отпуск',
'Enter_Annual_Leave' => 'Введите ежегодный отпуск',
'Remaining_leave' => 'Оставшийся отпуск',
'Employee_Details' => 'Сведения о сотруднике',
'Basic_Information' => 'Основная информация',
'Family_status' => 'Семейный статус',
'Choose_Family_status' => 'Выберите семейный статус',
'Employment_type' => 'Вид занятости',
'Select_Employment_type' => 'Выберите тип занятости',
'Enter_City' => 'Введите город',
'Province' => 'Область',
'Enter_Province' => 'Введите провинцию',
'Enter_Address' => 'Введите адрес',
'Enter_Zip_code' => 'Введите почтовый индекс',
'Zip_code' => 'Почтовый индекс',
'Hourly_rate' => 'Почасовая ставка',
'Enter_Hourly_rate' => 'Введите почасовую ставку',
'Basic_salary' => 'Базовая заработная плата',
'Enter_Basic_salary' => 'Введите базовую заработную плату',
'Social_Media' => 'Социальные сети',
'Skype' => 'Скайп',
'Enter_Skype' => 'Введите свой скайп',
'Facebook' => 'Фейсбук',
'Enter_Facebook' => 'Введите свой Фейсбук',
'WhatsApp' => 'ватсап',
'Enter_WhatsApp' => 'Введите свой ватсап',
'LinkedIn' => 'ЛинкедИн',
'Enter_LinkedIn' => 'Введите свой ЛинкедИн',
'Twitter' => 'Твиттер',
'Enter_Twitter' => 'Введите свой Твиттер',
'Experiences' => 'Опыт',
'bank_account' => 'банковский счет',
'Company_Name' => 'название компании',
'Location' => 'положение',
'Enter_location' => 'Введите местоположение',
'Enter_Description' => 'Введите описание',
'Bank_Name' => 'Имя банка',
'Enter_Bank_Name' => 'Введите название банка',
'Bank_Branch' => 'Филиал банка',
'Enter_Bank_Branch' => 'Филиал банка',
'Bank_Number' => 'Номер банка',
'Enter_Bank_Number' => 'Номер банка',
'Assigned_warehouses' => 'Присвоенные склады',
'Top_customers' => 'Лучшие клиенты',
'Attachment' => 'Вложение',
'view_employee' => 'просмотреть сотрудников',
'edit_employee' => 'редактировать сотрудников',
'delete_employee' => 'удалить сотрудников',
'Created_by' => 'Добавлено',
'Add_product_IMEI_Serial_number' => 'Добавить IMEI/серийный номер продукта',
'Product_Has_Imei_Serial_number' => 'Продукт имеет IMEI/серийный номер',
'IMEI_SN' => 'IMEI/SN',
'Shipments' => 'Отгрузки',
'delivered_to' => 'Доставлен в',
'shipment_ref' => 'Отгрузка',
'sale_ref' => 'Продажа Реф.',
'Edit_Shipping' => 'Изменить доставку',
'Packed' => 'упакованный',
'Shipped' => 'Отправленный',
'Delivered' => 'Доставленный',
'Cancelled' => 'Отменено',
'Shipping_status' => 'Статус отправки',
'Users_Report' => 'Отчет пользователей',
'stock_report' => 'Отчет о запасах',
'TotalPurchases' => 'Всего покупок',
'Total_quotations' => 'Всего котировок',
'Total_return_sales' => 'Общий возврат продаж',
'Total_return_purchases' => 'Всего возвращенных покупок',
'Total_transfers' => 'Всего переводов',
'Total_adjustments' => 'Всего корректировок',
'User_report' => 'Пользовательский отчет',
'Current_stock' => 'Текущий запас',
'product_name' => 'наименование товара',
'Total_Customers_Due' => 'Общая задолженность',
'Total_Suppliers_Due' => 'Общая задолженность',
'Some_warehouses' => 'Некоторые склады',
'All_Warehouses' => 'Все склады',
'Product_Cost' => 'Стоимость продукта',
'sms_settings' => 'Настройки смс',
'pos_settings' => 'Настройки POS',
'payment_gateway' => 'Платежный шлюз',
'mail_settings' => 'Настройки почты',
'Nexmo_SMS' => 'Nexmo SMS',
'TWILIO_SMS' => 'TWILIO SMS',
'Default_SMS_Gateway' => 'SMS-шлюз по умолчанию',
'module_settings' => 'Настройки модуля',
'Module_enabled_success' => 'Модуль успешно включен',
'Module_Disabled_success' => 'Модуль успешно отключен',
'update_settings' => 'Обновить настройки',
'Please_Upload_the_Correct_Module' => 'Пожалуйста, загрузите правильный модуль',
'Uploaded_Success' => 'Загружено успешно',
'Customer_details' => 'Сведения о клиенте',
'Edit_Customer' => 'Редактировать заказчика',
'Delete_Customer' => 'Удалить клиента',
'Pay_Due' => 'Оплатить должный',
'Paying_amount_is_greater_than_Total_Due' => 'Сумма платежа больше общей суммы платежа',
'Customer_Invoice' => 'счет-фактура клиента',
'This_Product_Not_For_Selling' => 'Этот продукт не для продажи',
'Welcome_to_your_Dashboard' => 'Добро пожаловать в вашу панель инструментов',
'Total_Payable' => 'Итого к оплате',
'Choose_Sale_Ref' => 'Выберите номер продажи',
'Please_Select_Sale' => 'Пожалуйста, выберите Распродажа',
'Sale_Ref' => 'Продажа Реф.',
'list_product_returns' => 'список возвратов продукции',
'Qty_return' => 'Кол-во возврата',
'Choose_Purchase_Ref' => 'Выберите покупку реф.',
'Purchase_Ref' => 'покупка реф',
'please_select_purchase' => 'Пожалуйста, выберите Покупка',
'qty_return_is_greater_than_qty_purchased' => 'Кол-во возврата больше, чем куплено',
'qty_return_is_greater_than_qty_sold' => 'Возврат количества больше, чем количество продано',
'Quantity_sold' => 'Кол-во продано',
'qty_purchased' => 'Кол-во куплено',
'Sender_Name' => 'Имя отправителя',
'The_module_must_be_uploaded_as_zip_file' => 'Модуль должен быть загружен в виде zip-файла',
'Upload_Module' => 'Выгрузить модуль',
'All_Modules_Installed' => 'Все модули установлены',
'Current_Version' => 'Текущая версия',
'You_already_have_the_latest_version' => 'У вас уже есть последняя версия',
'Update_Available' => 'Обновления доступны',
'Update_Log' => 'Журнал обновлений',
'Update_Now' => 'Обновить сейчас',
'View_Change_Log' => 'Просмотр журнала изменений',
'invoice_footer' => 'Нижний колонтитул счета',
'Please_Wait_for_the_update' => 'ПРИМЕЧАНИЕ. Дождитесь полного завершения обновления. Не рекомендуется закрывать браузер или останавливать процесс обновления.',
'Note_update' => 'Примечание. Обязательно сделайте резервную копию текущей версии и базы данных перед запуском обновления, чтобы восстановить их в случае ошибки.',
'Time_Zone' => 'Часовой пояс',
'Payment_note' => 'отмечает оплату',
'sale_note' => 'примечания к продаже',
'Total_Sale_Due' => 'Общая задолженность по продаже',
'Total_Sell_Return_Due' => 'Общий долг по возврату продаж',
'pay_all_sell_due_at_a_time' => 'погасить весь долг за один раз',
'pay_all_sell_return_due_at_a_time' => 'оплатить всю задолженность по возврату продажи за один раз',
'Gross_Profit' => 'Валовая прибыль',
'pay_all_purchase_due_at_a_time' => 'погасить всю задолженность по покупке единовременно',
'Delete_Provider' => 'Удалить провайдера',
'Edit_Provider' => 'Изменить провайдера',
'Provider_details' => 'Сведения о поставщике',
'Total_Purchase_Due' => 'Общий долг покупки',
'Provider_Credit_Note' => 'Кредитная нота',
'Customer_Credit_Note' => 'Кредитная нота',
'Sell_Return' => 'Продать Возврат',
'Purchase_Return' => 'Возврат покупки',
'Tax_Number' => 'Налоговый номер',
'Please_add_return_quantity' => 'Пожалуйста, добавьте количество возврата',
'Return_exist_for_the_Transaction' => 'Возврат существует для транзакции',
'qty_return_is_greater_than_Quantity_Remaining' => 'Возврат количества больше, чем оставшееся количество',
'products_refunded_alert' => 'Любые продукты с количеством, установленным на 0, не будут возвращены',
'pay_all_purchase_return_due_at_a_time' => 'оплатить всю задолженность по возврату покупки единовременно',
'Total_Purchase_Return_Due' => 'Общая задолженность по возврату покупки',
'Purchase_return_due' => 'Задолженность по возврату покупки',
'Sell_return_due' => 'Продать возврат из-за',
'product_report' => 'Отчет о продукте',
'product_sales_report' => 'отчет о продажах продукции',
'Product_purchases_report' => 'Отчет о покупках продуктов',
'Qty_sold' => 'Кол-во продано',
'Qty_purchased' => 'Кол-во куплено',
'Filter_by_warehouse' => 'Отфильтровать по складу',
'Enable_Print_Invoice' => 'Распечатать счет автоматически',
'Show_Warehouse' => 'посмотреть склад',
'credit_card_info' => 'Информация о кредитной карте',
'Saved_Credit_Card_Info' => 'Сохраненная информация о кредитной карте',
'Credit_card_changed_successfully' => 'Кредитная карта успешно изменена',
'notification_template' => 'Шаблон уведомления',
'email_notification' => 'Уведомление по электронной почте',
'sms_notification' => 'Уведомление по SMS',
'Change_product_details' => 'Изменить детали продукта',
'edit_tax_and_discount_and_shipping' => 'Изменить налог, скидку и доставку',
'Create_Quotation_with_Stock' => 'Создать квоту со складом',
'Variant_Name' => 'Название варианта',
'Variant_cost' => 'Стоимость варианта',
'Variant_price' => 'Цена варианта',
'Variant_code' => 'Код варианта',
'Please_wait_until_the_product_is_loaded' => 'Пожалуйста, подождите, пока продукт загружается',
'Choose_SMS_Gateway' => 'Выберите шлюз SMS',
'Notification_Client' => 'Уведомление клиента',
'Available_Tags' => 'Доступные теги',
'Email_Subject' => 'Тема письма',
'Email_body' => 'Тело электронного письма',
'Notification_Supplier' => 'Уведомление поставщика',
'sms_body' => 'Текст SMS',
'sms_templates' => 'SMS Templates',
'email_templates' => 'Email Templates',

'Edit_Project' => 'Редактировать проект',
'Projects' => 'Проекты',
'Enter_Project_Title' => 'Введите название проекта',
'Assigned_Employees' => 'Назначенные сотрудники',
'Choose_Team' => 'Выбрать команду',
'Create_Project' => 'Создать проект',
'Project_List' => 'Список проектов',
'Not_Started' => 'Не начато',
'In_Progress' => 'В процессе',
'On_Hold' => 'На удержании',
'Edit_Task' => 'Редактировать задачу',
'Tasks' => 'Задачи',
'Enter_Task_Title' => 'Введите название задачи',
'Choose_Project' => 'Выбрать проект',
'Project' => 'Проект',
'Create_Task' => 'Создать задачу',
'Task_List' => 'Список задач',
'Import_Purchases' => 'Импортировать покупки',
'Warranty_Period' => 'Срок гарантии',
'Days' => 'Дни',
'Months' => 'Месяцы',
'Years' => 'Годы',
'WarrantyTerms' => 'Условия гарантии',
'HasGuarantee' => 'Есть гарантия',
'Guarantee_Period' => 'Период гарантии',
'OpeningStock' => 'Начальный запас',
'Warranty_Guarantee_Tracking' => 'Отслеживание гарантии',
'PaymentOption' => 'Способ оплаты',
'AddAnotherPaymentOption' => 'Добавить способ оплаты',
'PopularTendered' => 'Популярные суммы',
'clear' => 'очистить',
'Pay' => 'Оплатить',
'Payment' => 'Платёж',
'TotalPaidExceedsGrandTotalForMultiPayment' => 'Общая сумма оплаты превышает итоговую стоимость',
'TotalPaying' => 'Общая сумма оплаты',
'ChangeReturn' => 'Сдача',
'AmountPaying' => 'Сумма оплаты',
'Appearance_Settings' => 'Настройки внешнего вида',
'app_name' => 'Название приложения',
'page_title_suffix' => 'Суффикс заголовка страницы',
'ChangeFavicon' => 'Изменить иконку (Favicon)',
'Languages' => 'Языки',
'Translations' => 'Переводы',
'Translations for' => 'Переводы для',
'Back' => 'Назад',
'Save All Changes' => 'Сохранить все изменения',
'Add New' => 'Добавить',
'Add New Translation' => 'Добавить новый перевод',
'Save' => 'Сохранить',
'Delete' => 'Удалить',
'Showing' => 'Показано',
'Translation added' => 'Перевод добавлен',
'Failed to add translation' => 'Не удалось добавить перевод',
'Translation updated' => 'Перевод обновлен',
'Failed to update' => 'Не удалось обновить',
'No changes to save' => 'Нет изменений для сохранения',
'Notice' => 'Уведомление',
'All changes saved successfully' => 'Все изменения успешно сохранены',
'Bulk save failed' => 'Ошибка пакетного сохранения',
'Please reload the page after saving translations to apply the changes.' => 'Пожалуйста, перезагрузите страницу после сохранения переводов, чтобы применить изменения.',
'Action_Blocked' => 'Действие заблокировано',
'You_cannot_delete_the_default_language' => 'Вы не можете удалить язык по умолчанию',
'Flag' => 'Флаг',
'Locale' => 'Локаль',
'Update Language' => 'Обновить язык',
'Add Language' => 'Добавить язык',
'Send_Email' => 'Отправить Email',
'Send_SMS' => 'Отправить SMS',
'total_sales' => 'Общий объем продаж',
'Opening_Stock' => 'Начальный запас',
'Subscription_Product' => 'Продукт по подписке',
'Subscriptions' => 'Подписки',
'Billing_Cycle' => 'Платёжный цикл',
'total_cycles' => 'Всего циклов',
'remaining_cycles' => 'Оставшиеся циклы',
'next_billing_date' => 'Следующая дата оплаты',
'Failed_to_update_subscription_status' => 'Не удалось обновить статус подписки',
'Subscription_status_updated_successfully' => 'Статус подписки успешно обновлён',
'Create_Susbscription' => 'Создать подписку',
'Price_Per_Cycle' => 'Цена за цикл',
'Price_Per_Unit' => 'Цена за единицу',
'Active' => 'Активна',
'Canceled' => 'Отменена',
'Completed' => 'Завершена',
'Subscription_details' => 'Детали подписки',
'Dynamic_Appearance' => 'Динамический внешний вид',
'Payment_Methods' => 'Способы оплаты',
'You_cant_edit_or_remove_default_payment_choices' => 'Вы не можете редактировать или удалять методы оплаты по умолчанию',
'Enter_Payment_Method' => 'Введите способ оплаты',
'Sales_by_Category' => 'Продажи по категориям',
'Sales_by_Brand' => 'Продажи по брендам',
'total_sales' => 'Общий объем продаж',
'Error_Logs' => 'Журналы ошибок',
'Occurred_At' => 'Произошло в',
'Details' => 'Подробности',
'Message' => 'Сообщение',
'Context' => 'Контекст',
'Report_Transactions' => 'Отчет о транзакциях',
'Payment_Method' => 'Способ оплаты',
'Customer_Provider' => 'Клиент/Поставщик',
'Sale_Purchase_Ref' => 'Ссылка на продажу/покупку',
'Reference' => 'Ссылка',
'Seller_report' => 'Отчет продавца',
'Seller' => 'Продавец',
'Create' => 'Создать',
'Banners' => 'Баннеры',
'Title' => 'Заголовок',
'Position' => 'Позиция',
'Recommended' => 'Рекомендуется',
'Uploaded' => 'Загружено',
'Aspect ratio differs from recommendation' => 'Соотношение сторон отличается от рекомендованного',
'Looks good' => 'Выглядит хорошо',
'Image' => 'Изображение',
'Disabled' => 'Отключено',
'Size guide' => 'Руководство по размеру',
'Tip' => 'Подсказка',
'Use 2x for retina (e.g., 2400×1200).' => 'Используйте масштаб 2x для Retina (напр., 2400×1200).',
'Saved_successfully' => 'Успешно сохранено',
'Store' => 'Магазин',
'Preview' => 'Предпросмотр',
'Updated' => 'Обновлено',
'Actions' => 'Действия',
'New_Collection' => 'Новая коллекция',
'Collection_Details' => 'Сведения о коллекции',
'Draft' => 'Черновик',
'Visible' => 'Видимый',
'Inactive' => 'Неактивно',
'Slug' => 'Слаг',
'Limit' => 'Лимит',
'Save_and_Close' => 'Сохранить и закрыть',
'Tip_reorder_products_for_priority' => 'Подсказка: измените порядок товаров, чтобы настроить приоритет отображения.',
'Products_in_Collection' => 'Товары в коллекции',
'selected' => 'выбрано',
'Display_limit' => 'Лимит отображения',
'Search_products' => 'Поиск товаров',
'No_results' => 'Нет результатов',
'variants' => 'варианты',
'Added' => 'Добавлено',
'Start_typing_to_search' => 'Начните печатать для поиска',
'Search_by_name_SKU_or_ID' => 'Поиск по названию, SKU или ID',
'No_products_in_collection_yet' => 'В этой коллекции пока нет товаров',
'Use_search_to_add_products' => 'Используйте поиск, чтобы добавить товары',
'Product' => 'Товар',
'Remove' => 'Удалить',
'Order_determines_display_priority' => 'Порядок определяет приоритет отображения.',
'Clear_all' => 'Очистить все',
'Confirm_Clear_All' => 'Вы уверены, что хотите очистить все выбранные товары?',
'Title_and_Slug_required' => 'Требуются заголовок и слаг.',
'Invalid' => 'Недействительно',
'Failed_to_load' => 'Не удалось загрузить.',
'Collection_saved_but_products_not_synced' => 'Коллекция сохранена, но синхронизировать товары не удалось.',
'Edit_Collection' => 'Редактировать коллекцию',
'Collections' => 'Коллекции',
'Search' => 'Поиск',
'New' => 'Новый',
'No_items' => 'Нет элементов',
'Order_saved' => 'Порядок сохранен.',
'Could_not_save_order' => 'Не удалось сохранить порядок.',
'Confirm_Delete_This_Item' => 'Удалить этот элемент?',
'Deleted_successfully' => 'Успешно удалено.',
'Delete_failed' => 'Не удалось удалить.',
'Messages' => 'Сообщения',
'Search_by_name_email_subject' => 'Поиск по имени, email или теме',
'Unread_only' => 'Только непрочитанные',
'Read' => 'Прочитанные',
'Unread' => 'Непрочитанные',
'From' => 'От',
'No_message_selected' => 'Сообщение не выбрано',
'Orders' => 'Заказы',
'To' => 'До',
'Clear' => 'Очистить',
'All_Status' => 'Все статусы',
'Order' => 'Заказ',
'Date' => 'Дата',
'Confirm' => 'Подтвердить',
'Keep' => 'Оставить',
'pending' => 'в ожидании',
'confirmed' => 'подтверждено',
'cancelled' => 'отменено',
'Confirm_this_order_Q' => 'Подтвердить этот заказ?',
'Confirm_to_sale_hint' => 'Это преобразует онлайн-заказ в продажу и спишет товар со склада.',
'Order_confirmed_as_sale' => 'Заказ подтвержден как продажа :ref',
'Order_confirmed' => 'Заказ подтвержден',
'Cancel_this_order_Q' => 'Отменить этот заказ?',
'Cancel_order_hint' => 'Заказ будет отмечен как отмененный.',
'Order_cancelled' => 'Заказ отменен',
'Operation_failed_try_again' => 'Операция не удалась. Попробуйте снова.',
'Insufficient_stock_for' => 'Недостаточно товара для:',
'Available' => 'Доступно',
'Required' => 'Требуется',
'Update_Status' => 'Обновить статус',
'No_actions_available' => 'Нет доступных действий',
'Warehouse' => 'Склад',
'Summary' => 'Сводка',
'Subtotal' => 'Промежуточный итог',
'Store_Basics' => 'Базовые настройки магазина',
'Enabled' => 'Включено',
'Store_Name' => 'Название магазина',
'Currency' => 'Валюта',
'Primary_Color' => 'Основной цвет',
'Secondary_Color' => 'Дополнительный цвет',
'Font_Family' => 'Семейство шрифтов',
'Contact' => 'Контакты',
'Contact_Email' => 'Контактный email',
'Contact_Phone' => 'Контактный телефон',
'Contact_Address' => 'Контактный адрес',
'Branding' => 'Брендинг',
'Logo' => 'Логотип',
'Favicon' => 'Фавикон',
'Hero_Header' => 'Заголовок хиро-блока',
'Hero_Title' => 'Заголовок хиро-блока',
'Hero_Subtitle' => 'Подзаголовок хиро-блока',
'Hero_Image' => 'Изображение хиро-блока',
'SEO' => 'SEO',
'SEO_Title' => 'SEO-заголовок',
'SEO_Description' => 'SEO-описание',
'Topbar_and_Footer' => 'Верхняя панель и подвал',
'Topbar_Text_Left' => 'Текст верхней панели (слева)',
'Topbar_Text_Right' => 'Текст верхней панели (справа)',
'Footer_Text' => 'Текст подвала',
'Social_Links' => 'Ссылки на соцсети',
'Add_Link' => 'Добавить ссылку',
'Homepage_Blocks' => 'Блоки главной страницы',
'Toggle_to_show_on_home_and_use_arrows_to_reorder' => 'Включите показ на главной и используйте стрелки для изменения порядка',
'Incomplete' => 'Не завершено',
'Collection' => 'Коллекция',
'Hero' => 'Хиро-блок',
'Newsletter' => 'Рассылка',
'Untitled' => 'Без названия',
'Unsaved_changes_may_be_lost' => 'Несохраненные изменения могут быть утеряны',
'Subscribers' => 'Подписчики',
'Subscribed At' => 'Дата подписки',
'Visit_Online_Store' => 'Перейти в интернет-магазин',
'Online_Orders' => 'Онлайн-заказы',
'Dead_Stock_Report' => 'Отчет по «мертвым» запасам',
'Period' => 'Период',
'Dimension' => 'Измерение',
'Last_30_days' => 'Последние 30 дней',
'Last_60_days' => 'Последние 60 дней',
'Last_90_days' => 'Последние 90 дней',
'By_Product' => 'По товару',
'By_Variant' => 'По варианту',
'Never_Moved' => 'Не перемещалось',
'OnHand' => 'В наличии',
'LastMovement' => 'Последнее перемещение',
'DaysSinceLastMovement' => 'Дней с последнего перемещения',
'Discount_Summary_Report' => 'Сводный отчет по скидкам',
'GroupBy' => 'Группировать по',
'Include_Points_Discounts' => 'Включать скидки по баллам',
'Allocate_Header_Discount' => 'Распределить скидку заголовка',
'Day' => 'День',
'User' => 'Пользователь',
'Line_Discount' => 'Скидка строки',
'Header_Discount' => 'Скидка заголовка',
'Allocated_Header' => 'Распределенная скидка заголовка',
'Total_Discount' => 'Общая скидка',
'By_Day' => 'По дням',
'By_User' => 'По пользователям',
'Last_7_days' => 'Последние 7 дней',
'Next' => 'Далее',
'Prev' => 'Назад',
'Draft_Invoices_Report' => 'Отчет по черновикам счетов',
'Only_Unconverted' => 'Только неконвертированные',
'Number' => 'Номер',
'AgeDays' => 'Возраст (дни)',
'days' => 'дней',
'All_time' => 'За все время',
'Inactive_Customers_Report' => 'Отчет по неактивным клиентам',
'LastPurchase' => 'Последняя покупка',
'DaysInactive' => 'Дней неактивности',
'Never_Purchased' => 'Никогда не покупал',
'Report' => 'Отчет',
'Stock_Adjustment_Report' => 'Отчет по корректировкам запасов',
'DateRange' => 'Диапазон дат',
'Refresh' => 'Обновить',
'Export_Excel' => 'Экспорт в Excel',
'Export_PDF' => 'Экспорт в PDF',
'Adjustments' => 'Корректировки',
'QtyAdded' => 'Добавленное количество',
'QtyRemoved' => 'Удаленное количество',
'NetQty' => 'Чистое количество',
'NetAdjustmentsOverTime' => 'Чистые корректировки во времени',
'AdjustmentTypes' => 'Типы корректировок',
'ByQuantity' => 'По количеству',
'ID' => 'ID',
'Ref' => 'Ссылка',
'Apply' => 'Применить',
'CustomRange' => 'Произвольный диапазон',
'Stock_Aging_Report' => 'Отчет по старению запасов',
'Buckets' => 'Интервалы',
'e.g. 30,60,90' => 'напр., 30,60,90',
'Comma_separated_cutoffs_default' => 'Пороговые значения через запятую, по умолчанию',
'Category' => 'Категория',
'LastInbound' => 'Последнее поступление',
'Bucket' => 'Интервал',
'By Product' => 'По товару',
'By Variant' => 'По варианту',
'Stock_Transfer_Report' => 'Отчет по перемещениям запасов',
'QuickRanges' => 'Быстрые диапазоны',
'MTD' => 'С начала месяца (MTD)',
'YTD' => 'С начала года (YTD)',
'Direction' => 'Направление',
'Inbound' => 'Входящее',
'Outbound' => 'Исходящее',
'Transfers' => 'Перемещения',
'Documents' => 'Документы',
'Lines' => 'Строки',
'QtyMoved' => 'Перемещенное количество',
'ValueMoved' => 'Перемещенная стоимость',
'AvgItemsPerTransfer' => 'Сред. позиций на перемещение',
'AvgValuePerTransfer' => 'Сред. стоимость на перемещение',
'TransfersOverTime' => 'Перемещения во времени',
'TopRoutes' => 'Лучшие маршруты',
'ByValue' => 'По стоимости',
'Value' => 'Стоимость',
'Tax_Summary_Report' => 'Сводный отчет по налогам',
'Tax_Rate' => 'Ставка налога',
'Taxable_Base' => 'Налогооблагаемая база',
'Tax_Collected' => 'Собрано налога',
'Effective_Rate' => 'Эффективная ставка',
'By_Tax_Rate' => 'По налоговой ставке',
'Zero_Sales_Products_Report' => 'Отчет по товарам без продаж',
'LastSale' => 'Последняя продажа',
'DaysSinceLastSale' => 'Дней с последней продажи',
'Never_Sold' => 'Никогда не продавался',
'Top_Suppliers_Report' => 'Отчет по лучшим поставщикам',
'Vendors' => 'Поставщики',
'QtyPurchased' => 'Закупленное количество',
'TotalSpend' => 'Общие расходы',
'TopSuppliersByValue' => 'Топ поставщиков по стоимости',
'TopSuppliersByQty' => 'Топ поставщиков по количеству',
'AvgPerPurchase' => 'Среднее на закупку',
'Customer_Ledger' => 'Лицевой счет клиента',
'Download_PDF' => 'Скачать PDF',
'Generating' => 'Генерация...',
'Sales_Grand' => 'Продажи (итог)',
'Sales_Paid' => 'Продажи (оплачено)',
'Sales_Due' => 'Продажи (к оплате)',
'Returns_Due' => 'Возвраты (к зачёту)',
'Payments_Total' => 'Платежи (итого)',
'Per_page' => 'На странице',
'Page' => 'Страница',
'Of' => 'из',
'Page_totals' => 'Итоги страницы',
'Page_total' => 'Итог страницы',
'Search_sales_ph' => 'Поиск продаж (ссылка, статус, статус оплаты, склад, клиент)',
'Search_payments_ph' => 'Поиск платежей (ссылка, дата, метод)',
'Search_quotations_ph' => 'Поиск предложений (ссылка, статус, склад, клиент)',
'Search_returns_ph' => 'Поиск возвратов (ссылка, статус, статус оплаты, ссылка продажи, склад)',
'Grand' => 'Итог',
'Grand_Total' => 'Общая сумма',
'Payment_Status' => 'Статус платежа',
'Shipping_Status' => 'Статус доставки',
'Payment_Ref' => 'Ссылка платежа',
'Method' => 'Метод',
'Quotation_Ref' => 'Ссылка предложения',
'Return_Ref' => 'Ссылка возврата',
'Is_Royalty_Eligible' => 'Имеет право на роялти',
'There_are' => 'Есть',
'Customers_without_ecommerce_notice' => 'клиенты, у которых нет учетной записи в интернет-магазине.',
'View_Details' => 'Посмотреть детали',
'Customer_Ledger' => 'Журнал клиента',
'Edit_Online_Store_Account' => 'Редактировать аккаунт интернет-магазина',
'Adjust_Customer_Points' => 'Изменить баллы клиента',
'Cancel' => 'Отмена',
'Save' => 'Сохранить',
'Draft' => 'Черновик',
'Recent_Drafts' => 'Недавние черновики',
'Available_Points' => 'Доступные баллы',
'Total_Cheque' => 'Всего по чекам',
'Total_Credit_Card' => 'Всего по кредитным картам',
'Total_Cash' => 'Всего наличными',
'Total_Amount_Paid' => 'Общая сумма оплачено',
'Total_Sales_Amount' => 'Общая сумма продаж',
'Search_Categories' => 'Поиск категорий',
'Search_Brands' => 'Поиск брендов',
'Show_Languages' => 'Показать языки',
'Payment_Purchases' => 'Платежи за покупки',
'PaymentsByMethod' => 'Платежи по методу',
'ByAmount' => 'По сумме',
'PaymentsOverTime' => 'Платежи с течением времени',
'DiscountsOverTime' => 'Скидки с течением времени',
'TaxesOverTime' => 'Налоги с течением времени',
'Featured_Product' => 'Рекомендуемый товар',
'ProductSummary' => 'Сводка по продукту',
'BasicInformation' => 'Основная информация',
'PricingAndTax' => 'Цены и налоги',
'Inventory' => 'Инвентарь',
'Variants' => 'Варианты',
'ComboProducts' => 'Комбинированные продукты',
'Options' => 'Опции',
'Enter_Product_Code' => 'Введите код продукта',
'ProductImage' => 'Изображение продукта',
'Supported_formats_JPG_PNG_GIF' => 'Поддерживаемые форматы: JPG, PNG, GIF',
'SearchProduct' => 'Поиск продукта',
'ProductName' => 'Название продукта',
'StandardProduct' => 'Стандартный продукт',
'VariableProduct' => 'Переменный продукт',
'ServiceProduct' => 'Сервисный продукт',
'ComboProduct' => 'Комбинированный продукт',
'Enter_warranty_terms' => 'Введите условия гарантии',
'Create_a_new_product_with_all_details' => 'Создайте новый продукт со всеми деталями',
'TotalCost' => 'Общая стоимость',
'Barcode_must_be_unique' => 'Штрихкод должен быть уникальным',
'Product_name_minimum_3_characters' => 'Название продукта должно содержать не менее 3 символов',
'Category_is_required' => 'Категория обязательна',
'Wholesale_Price' => 'Оптовая цена',
'Enter_Wholesale_Price' => 'Введите оптовую цену',
'Minimum_Selling_Price' => 'Минимальная цена продажи',
'Enter_Minimum_Selling_Price' => 'Введите минимальную цену продажи',
'Price_below_min_not_allowed' => 'Цена ниже минимальной не разрешена',
'Retail Price' => 'Розничная цена',
'Enter_the_Variant' => 'Введите вариант',
'Min_Selling_Price' => 'Минимальная цена продажи',
'Are_you_sure_you_want_to_duplicate_this_product' => 'Вы уверены, что хотите продублировать этот продукт?',
'Variant_Name' => 'Название варианта',
'POS_Settings' => 'Настройки POS',
'Fullscreen' => 'Полноэкранный режим',
'Select_Warehouse' => 'Выберите склад',
'Select_Customer' => 'Выберите клиента',
'Scan' => 'Сканировать',
'Today_Sales' => 'Продажи за сегодня',
'Loading' => 'Загрузка...',
'Open' => 'Открыто',
'Closed' => 'Закрыто',
'Convert' => 'Преобразовать',
'Converted' => 'Преобразовано',
'InsufficientStock' => 'Недостаточно товара для',
'pos.Checkout' => 'Оформление заказа',
'pos.items' => 'товары',
'pos.No_items_added' => 'Товары не добавлены',
'pos.Select_products_from_right_panel' => 'Выберите продукты в правой панели',
'pos.Edit' => 'Редактировать',
'pos.Remove' => 'Удалить',
'pos.Decrease' => 'Уменьшить',
'pos.Increase' => 'Увеличить',
'pos.Available_Products' => 'Доступные продукты',
'pos.All_Categories' => 'Все категории',
'pos.All_Brands' => 'Все бренды',
'pos.Reset' => 'Сбросить',
'pos.Add_to_cart' => 'Добавить в корзину',
'pos.Previous_Page' => 'Предыдущая страница',
'pos.Go_to_page' => 'Перейти на страницу',
'pos.Next_Page' => 'Следующая страница',
'pos.Home' => 'Главная',
'pos.Clear_all_items' => 'Очистить все товары',
'pos.Drafts_list' => 'Список черновиков',
'pos.Recent_Drafts' => 'Недавние черновики',
'pos.Hold_this_sale' => 'Удержать эту продажу',
'pos.Saving' => 'Сохранение...',
'pos.Hold' => 'Удержать',
'pos.Total_Payable' => 'Итого к оплате',
'pos.Complete_and_process_payment' => 'Завершить и обработать оплату',
'pos.Processing' => 'Обработка...',
'pos.Pay_Now' => 'Оплатить сейчас',
'pos.Subtotal' => 'Промежуточный итог',
'pos.Tax' => 'Налог',
'pos.Discount' => 'Скидка',
'pos.Shipping' => 'Доставка',
'pos.Grand_Total' => 'Общая сумма',
'pos.No_products_found' => 'Продукты не найдены',
'pos.Page' => 'Страница',
'pos.products' => 'продукты',
'pos.will_be_applied' => 'будет применено',
'Payment_Sent' => 'Платёж отправлен',
'Payment_Received' => 'Платёж получен',
'Welcome_back_message' => 'Добро пожаловать обратно, {username}! Вот, что происходит сегодня.',
'No_dashboard_permission' => 'У вас нет разрешения для просмотра панели управления.',
'Edit_Payment' => 'Редактировать оплату',
'Payment_Checkout' => 'Оформление оплаты',
'Transaction_Summary' => 'Сводка транзакции',
'Total_Amount' => 'Общая сумма',
'Payment_Breakdown' => 'Детали оплаты',
'Paying' => 'Оплата',
'Balance' => 'Баланс',
'Change' => 'Сдача',
'Payment' => 'Оплата',
'Amount' => 'Сумма',
'Account' => 'Счёт',
'Select_Account' => 'Выбрать счёт',
'Credit_Card' => 'Кредитная карта',
'Use_Saved_Credit_Card' => 'Использовать сохранённую карту',
'Enter_New_Credit_Card' => 'Введите новую кредитную карту',
'Saved_Credit_Cards' => 'Сохранённые кредитные карты',
'Brand' => 'Бренд',
'Last4' => 'Последние 4 цифры',
'Exp' => 'Срок действия',
'Card' => 'Карта',
'Default' => 'По умолчанию',
'Selected' => 'Выбрано',
'Use_This' => 'Использовать эту',
'Card_Info_Secure_Stripe' => 'Информация о вашей карте обрабатывается безопасно через Stripe',
'Add_Payment_Method' => 'Добавить способ оплаты',
'Payment_Date' => 'Дата оплаты',
'Payment_Note' => 'Примечание к оплате',
'Add_Payment_Note' => 'Добавить примечание к оплате',
'Sale_Note' => 'Примечание к продаже',
'Add_Sale_Note' => 'Добавить примечание к продаже',
'Send_Email_Receipt' => 'Отправить квитанцию по электронной почте',
'Send_SMS_Receipt' => 'Отправить квитанцию по SMS',
'Cancel' => 'Отмена',
'Update_Payment' => 'Обновить оплату',
'Complete_Payment' => 'Завершить оплату',
'Processing' => 'Обработка...',
'Please_enter_credit_card_details' => 'Пожалуйста, введите данные кредитной карты',
'Please_select_Account' => 'Пожалуйста, выберите счёт',
'WooCommerce' => 'WooCommerce',
'Store_URL' => 'URL магазина',
'Enter_Store_URL' => 'Введите URL магазина',
'Consumer_Key' => 'Ключ потребителя',
'Consumer_Secret' => 'Секрет потребителя',
'Enable_Auto_Sync' => 'Включить авто-синхронизацию',
'Enable' => 'Включить',
'Sync_Interval' => 'Интервал синхронизации',
'Choose_Interval' => 'Выберите интервал',
'Connection_Status' => 'Статус подключения',
'Test_Connection' => 'Проверить подключение',
'Testing' => 'Проверка',
'Connected' => 'Подключено',
'Disconnected' => 'Отключено',
'Unknown' => 'Неизвестно',
'Last_Sync' => 'Последняя синхронизация',
'Last_Sync_Time' => 'Время последней синхронизации',
'Total_Products' => 'Всего товаров',
'Synced_Products' => 'Синхронизированные товары',
'Not_Synced' => 'Не синхронизировано',
'Run_Manual_Sync_Now' => 'Запустить ручную синхронизацию сейчас',
'Sync_Only_Unsynced' => 'Синхронизировать только несинхронизированные',
'Sync_Products' => 'Синхронизировать товары',
'POS_to_Woo' => 'POS → WooCommerce',
'POS_to_WooCommerce' => 'POS → WooCommerce',
'WooCommerce_to_POS' => 'WooCommerce → POS',
'Sync_Completed' => 'Синхронизация завершена',
'Sync_Failed' => 'Синхронизация не удалась',
'Sync_Orders' => 'Синхронизировать заказы',
'Last_Synced_Orders' => 'Последние синхронизированные заказы',
'Sync_Categories' => 'Синхронизировать категории',
'Pull_From_Woo' => 'Импорт из WooCommerce',
'Save_Mapping' => 'Сохранить соответствие',
'Woo_ID' => 'ID Woo',
'Unsynced_Product_Count' => 'Количество несинхронизированных товаров',
'Last_5_Log_Entries' => 'Последние 5 записей журнала',
'Sync_Stock_Now' => 'Синхронизировать склад сейчас',
'In_Stock' => 'В наличии',
'Out_of_Stock' => 'Нет в наличии',
'Syncing_Products' => 'Синхронизируются товары',
'Errors' => 'Ошибки',
'View_Logs' => 'Просмотреть журналы',
'Hourly' => 'Каждый час',
'Daily' => 'Ежедневно',
'Connection_successful' => 'Подключение успешно',
'Connection_failed' => 'Подключение не удалось',
'Not_Available' => 'Недоступно',
'Opening_Balance' => 'Начальный баланс',
'Counted_Cash' => 'Подсчитанная наличность',
'Closing_Notes' => 'Примечания при закрытии',
'Quickbooks_Sync' => 'Синхронизация QuickBooks',
'WooCommerce_Settings' => 'Настройки WooCommerce',
'Status_Overview' => 'Обзор статуса',
'QuickBooks' => 'QuickBooks',
'Connection' => 'Подключение',
'Clients_Sync' => 'Синхронизация клиентов',
'Audit' => 'Аудит',
'Environment' => 'Окружение',
'Redirect_URI_active' => 'URI перенаправления (активен)',
'Redirect_URI' => 'URI перенаправления',
'Callback' => 'Callback',
'Realm_ID' => 'ID Realm',
'Connect_to_QuickBooks' => 'Подключиться к QuickBooks',
'Disconnect' => 'Отключиться',
'Not_connected_to_QuickBooks' => 'Не подключено к QuickBooks.',
'Please_connect_before_syncing_clients' => 'Пожалуйста, подключитесь перед синхронизацией клиентов.',
'Total_Clients' => 'Всего клиентов',
'Synced' => 'Синхронизировано',
'Sync_All_Clients' => 'Синхронизировать всех клиентов',
'Syncing' => 'Синхронизация...',
'Bulk_sync_unsynced_clients' => 'Массовая синхронизация несинхронизированных клиентов',
'Overall_progress' => 'Общий прогресс',
'Search_unsynced_by_name_email' => 'Поиск несинхронизированных по имени/почте...',
'Refresh_Stats' => 'Обновить статистику',
'Refresh_List' => 'Обновить список',
'All_clients_are_synced' => 'Все клиенты синхронизированы',
'Sync' => 'Синхронизировать',
'Sync_Selected' => 'Синхронизировать выбранные',
'Clear_Selection' => 'Очистить выбор',
'Added' => 'Добавлено',
'Show_failures' => 'Показать ошибки',
'no_name' => 'без имени',
'Notes' => 'Примечания',
'Only_clients_without_quickbooks_customer_id_are_candidates' => 'Кандидатами являются только клиенты без quickbooks_customer_id (пусто или NULL).',
'If_matching_email_exists_reuse_instead_of_duplicate' => 'Если совпадающий e-mail уже есть в QBO, используем его повторно, а не создаём дубликат.',
'When' => 'Когда',
'Operation' => 'Операция',
'Sale' => 'Продажа',
'Realm' => 'Realm',
'Env' => 'Окружение',
'Development' => 'Разработка',
'Production' => 'Продакшн',
'Sandbox_company_test_data' => 'Тестовая компания (sandbox).',
'Live_QB_company' => 'Рабочая компания QuickBooks (реальные клиенты и счета).',
'Client_ID' => 'ID клиента',
'Client_Secret' => 'Секрет клиента',
'Income_Account_Name' => 'Название счета доходов',
'Income_Account_Name_Hint' => 'Если оставить пустым, по умолчанию будет использовано «Sales of Product Income». При синхронизации будет выполнен поиск аккаунта с точно таким именем в QBO; если он не найден, будет использовано значение по умолчанию.',
'From_Intuit_Keys_Identifies_App' => 'Из Intuit Developer → Keys. Это идентифицирует ваше приложение в QuickBooks.',
'Paste_value_exactly_as_in_app_keys' => 'Вставьте значение точно так, как указано в ключах приложения (Development/Production).',
'From_Intuit_Keys_Keep_private_rotate' => 'Из Intuit Developer → Keys. Храните их в секрете. Поменяйте, если подозреваете утечку.',
'Must_match_Intuit_redirect_exactly' => 'Должно точно совпадать со значением redirect в вашем приложении Intuit (схема, хост, путь).',
'Typical' => 'Типично',
'Add_this_exact_URL_on_Keys_page_too' => 'Добавьте этот точный URL также на странице Keys.',
'Realm_ID_Hint' => 'ID компании (realmId). Необязательно — устанавливается автоматически после подключения. Заполняйте только если знаете его и хотите привязать к конкретной компании.',
'Failed_to_load_status' => 'Не удалось загрузить статус',
'Failed_to_disconnect' => 'Не удалось отключиться',
'Failed_to_load_settings' => 'Не удалось загрузить настройки',
'Settings_saved_to_env' => 'Настройки сохранены в .env',
'Failed_to_save_settings' => 'Не удалось сохранить настройки',
'Failed_to_load_audit_logs' => 'Не удалось загрузить журналы аудита',
'Failed_to_load_client_stats' => 'Не удалось загрузить статистику клиентов',
'Failed_to_load_unsynced_clients' => 'Не удалось загрузить несинхронизированных клиентов',
'Connect_to_QuickBooks_first' => 'Сначала подключитесь к QuickBooks',
'No_clients_were_synced' => 'Клиенты не были синхронизированы',
'Failed_to_sync_clients' => 'Не удалось синхронизировать клиентов',
'Selected' => 'Выбрано',
'Sync_selected_failed' => 'Синхронизация выбранного не удалась',
'Client_sync_attempted' => 'Попытка синхронизации клиента',
'Client_sync_failed' => 'Синхронизация клиента не удалась',
'Clients' => 'Клиенты',
'Stock' => 'Запасы',
'Clear_Logs' => 'Очистить журналы',
'Connect' => 'Подключить',
'Client' => 'Клиент',
'Level' => 'Уровень',
'Opened' => 'Открыто',
'Customer_Screen' => 'Экран клиента',
'Store_Logo' => 'Логотип магазина',
'Welcome' => 'Добро пожаловать',
'Thank_you_for_shopping_with_us' => 'Спасибо за покупку у нас',
'Waiting_for_Items' => 'Ожидание товаров',
'Items_will_appear_here_as_added_to_cart' => 'Товары будут отображаться здесь при добавлении в корзину',
'Thank_you_for_your_purchase' => 'Спасибо за вашу покупку!',
'Sale_Completed_Thank_You' => 'Продажа завершена. Спасибо!',
'Customer_Display_Configuration' => 'Настройка клиентского дисплея',
'Customer_Display_Configuration_Subtitle' => 'Настройте и управляйте дисплеем для клиентов в POS-системе',
'Generate_Access_Token' => 'Создать токен доступа',
'Generate_New_Token' => 'Создать новый токен',
'Generating' => 'Генерация',
'Create_secure_token_info' => 'Создайте безопасный токен для клиентского дисплея. Токены действительны 24 часа в целях безопасности.',
'Display_URL' => 'URL дисплея',
'Display_URL_Placeholder' => 'Здесь появится URL дисплея',
'Copy_to_clipboard' => 'Копировать в буфер обмена',
'Copy' => 'Копировать',
'Click_input_to_select_then_use_copy_button' => 'Кликните поле для выбора, затем используйте кнопку «Копировать»',
'QR_Code' => 'QR-код',
'Scan_QR_to_open_display' => 'Сканируйте этот QR-код, чтобы открыть дисплей клиента на любом устройстве',
'No_Token_Generated_Yet' => 'Токен ещё не создан',
'Click_Generate_New_Token_to_create' => 'Нажмите «Создать новый токен», чтобы получить URL дисплея и QR-код',
'How_to_Use' => 'Как использовать',
'Open_URL_on_display_device' => 'Откройте URL на устройстве, отображающем клиентский дисплей',
'Items_appear_realtime' => 'Товары из корзины POS будут отображаться в реальном времени',
'Display_updates_automatically' => 'Дисплей автоматически обновляется при добавлении товаров',
'Perfect_for_showing_purchases' => 'Идеально для показа покупок клиентам',
'Security' => 'Безопасность',
'Tokens_secure_time_limited' => 'Токены безопасны и имеют ограниченный срок действия',
'Token_expires_24h' => 'Каждый токен автоматически истекает через 24 часа',
'Generate_new_tokens_anytime' => 'Создавайте новые токены в любое время для ротации доступа',
'Old_tokens_invalid_immediately' => 'Старые токены сразу становятся недействительными',
'Features' => 'Функции',
'Real_time_item_updates' => 'Обновления товаров в реальном времени',
'Automatic_cart_sync' => 'Автоматическая синхронизация корзины',
'Theme_support_dark_light' => 'Поддержка тёмной и светлой темы',
'Professional_responsive_design' => 'Профессиональный адаптивный дизайн',
'Troubleshooting' => 'Устранение неисправностей',
'Display_not_connecting_Q' => 'Дисплей не подключается?',
'Display_not_connecting_A' => 'Убедитесь, что устройство в той же сети, и используйте полный URL, включая токен.',
'Items_not_updating_Q' => 'Товары не обновляются?',
'Items_not_updating_A' => 'Убедитесь, что POS-система запущена и URL дисплея используется активно.',
'Token_expired_Q' => 'Токен истёк?',
'Token_expired_A' => 'Создайте новый токен и обновите устройство дисплея новым URL.',
'Failed_to_generate_token' => 'Не удалось создать токен',
'Failed_to_copy_to_clipboard' => 'Не удалось скопировать в буфер обмена',
'Copied_to_clipboard' => 'Скопировано в буфер обмена',
'Scan_URL_with_your_camera' => 'Отсканируйте URL камерой',
'Synced_Categories' => 'Синхронизированные категории',
'Total_Categories' => 'Всего категорий',
'Reset_Sync_State' => 'Сбросить состояние синхронизации',
'WP_Username_Optional' => 'Имя пользователя WP (необязательно)',
'Used_for_media_upload_fallback' => 'Используется как резерв для загрузки медиафайлов',
'WP_Application_Password_Optional' => 'Пароль приложения WP (необязательно)',
'Create_from_WordPress_Profile' => 'Создать из профиля WordPress',
'Loyalty_Eligible' => 'Допущен к программе лояльности',
'Loyalty_Points_Help' => 'Если включено, этот клиент будет получать бонусные баллы за подходящие покупки и сможет использовать их позже.',
'Assets' => 'Активы',
'Assets_List' => 'Список Активов',
'Add_Asset' => 'Добавить Актив',
'Edit_Asset' => 'Редактировать Актив',
'Tag' => 'Тег',
'Serial' => 'Серийный Номер',
'Purchase_Date' => 'Дата Покупки',
'Purchase_Cost' => 'Стоимость Покупки',
'Sale_Price' => 'Цена Продажи',
'In_Use' => 'В Использовании',
'Maintenance' => 'Обслуживание',
'Retired' => 'Выведен из эксплуатации',
'AreYouSure' => 'Вы уверены?',
'Asset_Category' => 'Категория Активов',
'Please_enter_points_to_convert' => 'Пожалуйста, введите количество баллов для конвертации',
'Entered_points_exceed_available' => 'Введённые баллы превышают доступное количество',
'Unconverted' => 'Не конвертировано',
'Zero_total_no_payment_required' => 'Общая сумма ноль, оплата не требуется',
'cannot_be_negative' => 'не может быть отрицательным',
'Customer_Loyalty_Points_Report' => 'Отчёт о Баллах Лояльности Клиентов',
'Points_Earned' => 'Заработанные Баллы',
'Points_Redeemed' => 'Использованные Баллы',
'Points_Balance' => 'Баланс Баллов',
'Asset_Management' => 'Управление Активами',
'Asset_Categories' => 'Категории Активов',
'Damages' => 'Повреждения',
'Create_Damage' => 'Создать Повреждение',
'Edit_Damage' => 'Редактировать Повреждение',
'Damage_Detail' => 'Детали Повреждения',
'Product_Not_Found' => 'Товар не найден',
'Cash_Flow_Report' => 'Отчёт о Движении Денежных Средств',
'TotalInflow' => 'Общий Приток',
'TotalOutflow' => 'Общий Отток',
'NetCashFlow' => 'Чистый Денежный Поток',
'Inflow' => 'Приток',
'Outflow' => 'Отток',
'Net' => 'Чистый',
'Group' => 'Группа',
'Inflow_vs_Outflow_by_Group' => 'Приток и Отток по Группам',
'NetCashFlowOverTime' => 'Чистый Денежный Поток со Временем',
'PaymentMethod' => 'Способ Оплаты',
'AllWarehouses' => 'Все Склады',
'AllAccounts' => 'Все Счета',
'AllPaymentMethods' => 'Все Способы Оплаты',
'EXCEL' => 'Excel',
'Return_Ratio_Report' => 'Отчёт о Коэффициенте Возвратов',
'Negative_Stock_Report' => 'Отчёт об Отрицательных Остатках',
'Sales_Tax' => 'Налог с Продаж',
'Purchase_Tax' => 'Налог на Покупки',
'Total_Sales' => 'Общий Объём Продаж',
'Sale_Returns' => 'Возвраты Продаж',
'Net_Sales' => 'Чистые Продажи',
'Output_Tax' => 'Выходной Налог',
'Total_Purchases' => 'Общий Объём Покупок',
'Purchase_Returns' => 'Возвраты Покупок',
'Net_Purchases' => 'Чистые Покупки',
'Input_Tax' => 'Входной Налог',
'Net_Tax' => 'Чистый Налог',
'Taxable_Sales' => 'Облагаемые Продажи',
'Taxable_Purchases' => 'Облагаемые Покупки',
'Tax_Payable' => 'Налог, подлежащий уплате государственным органам',
'Tax_Refund' => 'Ожидаемый возврат налога',
'Failed_Load_Tax_Summary' => 'Не удалось загрузить сводку по налогам. Пожалуйста, попробуйте снова.',
'Chart_of_Accounts_Title' => 'План счетов',
'Chart_of_Accounts_Subtitle' => 'Управляйте структурой главной книги с иерархией родитель/дочерний.',
'Import_Chart_of_Accounts' => 'Импорт плана счетов',
'Only_CSV_allowed' => 'Файл должен быть в формате CSV',
'Parent_Code' => 'Код родителя',
'Yes' => 'Да',
'No' => 'Нет',
'Edit_Account' => 'Редактировать счёт',
'New_Account' => 'Новый счёт',
'Example_Code' => 'например: 1000',
'Example_Name' => 'например: Касса',
'Select_Type' => 'Выберите тип',
'Asset' => 'Актив',
'Liability' => 'Обязательство',
'Equity' => 'Капитал',
'Income' => 'Доход',
'Expense' => 'Расход',
'Parent' => 'Родитель',
'Type' => 'Тип',
'None' => 'Нет',
'Saving' => 'Сохранение...',
'Delete_Account_Title' => 'Удалить счёт',
'Delete_Account_Question' => 'Вы уверены, что хотите удалить?',
'Delete_Account_Warning' => 'Это действие навсегда удалит счёт.',
'Account_Updated' => 'Счёт обновлён',
'Account_Created' => 'Счёт создан',
'Operation_Failed' => 'Операция не удалась',
'Deleted_Successfully' => 'Успешно удалено',
'Delete_Failed' => 'Ошибка при удалении',
'Accounting_Dashboard_Title' => 'Расширенный учёт',
'Accounting_Dashboard_Subtitle' => 'Обзор и быстрые действия',
'Accounts' => 'Счета',
'Journal_Entries_30d' => 'Журнальные записи (30 дней)',
'Income_30d' => 'Доходы (30 дней)',
'Expense_30d' => 'Расходы (30 дней)',
'Chart_of_Accounts_Link' => 'План счетов',
'Journal_Entries_Link' => 'Журнальные записи',
'Trial_Balance_Link' => 'Оборотно-сальдовая ведомость',
'Profit_Loss_Link' => 'Отчёт о прибылях и убытках',
'Balance_Sheet_Link' => 'Баланс',
'Tax_Summary_Link' => 'Налоговая сводка',
'Journal_Entries_Title' => 'Журнальные записи',
'Journal_Entries_Subtitle' => 'Создавайте ручные записи и просматривайте опубликованные журналы.',
'Post' => 'Провести',
'Description_Placeholder' => 'Описание',
'Select_Account' => 'Выберите счёт',
'Add_Line' => 'Добавить строку',
'Total_Debit' => 'Общий дебет',
'Total_Credit' => 'Общий кредит',
'Balanced' => 'Сбалансировано',
'Not_Balanced' => 'Не сбалансировано',
'Journal_Number' => 'Журнал № %{number}',
'Debit' => 'Дебет',
'Credit' => 'Кредит',
'Memo' => 'Примечание',
'Source' => 'Источник',
'Journal_Status_Posted' => 'Проведено',
'Journal_Status_Draft' => 'Черновик',
'Complete_Lines_Message' => 'Пожалуйста, заполните обязательные поля в строках',
'Validation' => 'Проверка',
'Entry_Updated' => 'Запись обновлена',
'Entry_Created_Draft' => 'Запись создана (черновик)',
'Posted_Successfully' => 'Успешно проведено',
'Post_Failed' => 'Ошибка при проведении',
'Delete_Draft_Entry_Question' => 'Удалить эту черновую запись?',
'At_Least_One_Line' => 'Требуется как минимум одна строка',
'Notice' => 'Уведомление',
'Balance_Sheet_Title' => 'Баланс',
'Balance_Sheet_Subtitle' => 'Активы = Обязательства + Капитал',
'As_Of' => 'По состоянию на',
'Liabilities' => 'Обязательства',
'Balance_Check' => 'Проверка:',
'Profit_Loss_Title' => 'Отчёт о прибылях и убытках',
'Profit_Loss_Subtitle' => 'Обзор результатов за период.',
'Income' => 'Доход',
'Expense' => 'Расход',
'Net_Profit' => 'Чистая прибыль',
'Search_accounts' => 'Поиск счетов',
'All' => 'Все',
'Apply' => 'Применить',
'Total_Income' => 'Общий доход',
'Total_Expense' => 'Общие расходы',
'Trial_Balance_Title' => 'Оборотно-сальдовая ведомость',
'Trial_Balance_Subtitle' => 'Свод дебета/кредита по счетам.',
'Total' => 'Итого',
'attendance_summary' => 'Сводка посещаемости',
'Enable_ZATCA_QR_on_Sales_Receipts' => 'Включить QR-код ZATCA на кассовых чеках',
'VAT_Number' => 'Номер НДС',
'CompanyNameArabic' => 'Название компании (арабский)',
'Default_Tax' => 'Налог по умолчанию',
'Service_Maintenance' => 'Сервис и обслуживание',
'Service_Jobs' => 'Сервисные задания',
'Service_Technicians' => 'Сервисные техники',
'Checklists' => 'Контрольные списки',
'Maintenance_History' => 'История обслуживания',
'Create_Service_Job' => 'Создать сервисное задание',
'Edit_Service_Job' => 'Редактировать сервисное задание',
'Service_Item' => 'Сервисный объект / Транспорт',
'Job_Type' => 'Тип задания',
'Technician' => 'Техник',
'Technician_Details' => 'Данные техника',
'Scheduled_Date' => 'Запланированная дата',
'Checklist' => 'Контрольный список',
'No_checklist_items_defined' => 'Элементы контрольного списка ещё не определены',
'Checklist_Categories' => 'Категории контрольного списка',
'Checklist_Items' => 'Элементы контрольного списка',
'Item_Name' => 'Название элемента',
'Total_Items' => 'Всего элементов',
'Completed_Items' => 'Завершённые элементы',
'Customer_Maintenance_History' => 'История обслуживания клиента',
'Service_Jobs_Report' => 'Отчёт по сервисным заданиям',
'Checklist_Completion_Report' => 'Отчёт о выполнении контрольных списков',
'Customer_Maintenance_History_Report' => 'Отчёт об истории обслуживания клиента',
'Attach_Documents' => 'Прикрепить документы',
'Upload_Documents' => 'Загрузить документы',
'Choose_files_or_drop_them_here' => 'Выберите файлы или перетащите их сюда...',
'Drop_files_here' => 'Перетащите файлы сюда...',
'Attached_Documents' => 'Прикреплённые документы',
'File_Name' => 'Имя файла',
'Size' => 'Размер',
'Uploaded_Date' => 'Дата загрузки',
'Documents' => 'Документы',
'Failed_to_load_documents' => 'Не удалось загрузить документы',
'Please_select_files' => 'Пожалуйста, выберите файлы',
'Documents_uploaded_successfully' => 'Документы успешно загружены',
'Failed_to_upload_documents' => 'Не удалось загрузить документы',
'Failed_to_download_document' => 'Не удалось скачать документ',
'pos.Offline_Mode' => 'Офлайн-режим',
'pos.Sync_Offline_Sales' => 'Синхронизировать офлайн-продажи',
'pos.Offline_Sale_Saved' => 'Продажа сохранена офлайн. Она будет синхронизирована при подключении к интернету.',
'pos.CreditCard_Requires_Online' => 'Оплата кредитной картой требует подключения к интернету',
'CurrentPassword' => 'Текущий пароль',
'Newpassword' => 'Новый пароль',
'ConfirmPassword' => 'Подтвердите пароль',
'PasswordUpdated' => 'Пароль успешно обновлён',
'CurrentPasswordIncorrect' => 'Текущий пароль неверен',
'ChangePassword' => 'Изменить пароль',
'UpdatePassword' => 'Обновить пароль',
'Bookings' => 'Бронирования',
'Booking_List' => 'Список бронирований',
'Create_Booking' => 'Создать бронирование',
'Edit_Booking' => 'Редактировать бронирование',
'Booking_Details' => 'Детали бронирования',
'Start_Time' => 'Время начала',
'End_Time' => 'Время окончания',
'Not_Applicable' => 'Не применяется',
'Choose_Product' => 'Выбрать продукт',
'Calendar_View' => 'Календарный вид',
'Profit_and_Loss' => 'Прибыль и убытки',
'Choose_Sub_Category' => 'Выбрать подкатегорию',
'SubCategory' => 'Подкатегория',
'AddedBy' => 'Добавлено',
'General' => 'Общие',
'Localization' => 'Локализация',
'Defaults' => 'По умолчанию',
'Tax_Pricing' => 'Налоги и цены',
'Receipt_Display_Options' => 'Параметры отображения чека',
'POS_receipt_configuration' => 'Настройка чека POS',
'the_options_below_change_what_appears_on_the_printed_receipt' => 'Параметры ниже изменяют информацию, отображаемую на печатном чеке',
'to_keep_current_value' => 'чтобы сохранить текущее значение',
'Enable_to_remove_stored_credentials' => 'Включить для удаления сохранённых учётных данных',
'No_backups_found' => 'Резервные копии не найдены',
'Click_Generate_Backup_to_create_your_first_backup' => 'Нажмите «Создать резервную копию», чтобы создать первую копию',
'Backup_generated_successfully' => 'Резервная копия успешно создана',
'Failed_to_generate_backup' => 'Не удалось создать резервную копию',
'ZATCA_Enabled' => 'ZATCA включён',
'POS_Receipt' => 'Чек POS',
'POS_Settings' => 'Настройки POS',
'Quick_Add_Customer' => 'Быстрое добавление клиента',
'Enable_Quick_Add_Customer_popup_in_POS' => 'Включить всплывающее окно быстрого добавления клиента в POS',
'Barcode_Scanning_Sound' => 'Звук сканирования штрихкода',
'Enable_sound_when_scanning_barcodes_in_POS' => 'Включить звук при сканировании штрихкодов в POS',
'Show_Product_Images_in_POS' => 'Показывать изображения товаров в POS',
'Show_hide_product_images_in_POS_product_listing' => 'Показать/Скрыть изображения товаров в списке POS',
'Show_Stock_Quantity_in_POS' => 'Показывать количество на складе в POS',
'Show_hide_stock_quantity_in_POS' => 'Показать/Скрыть количество на складе в POS',
'Enable_Hold_Sales' => 'Включить отложенные продажи',
'Enable_disable_Hold_Sales_feature_in_POS' => 'Включить/Отключить функцию отложенных продаж в POS',
'Enable_Customer_Points_in_POS' => 'Включить бонусные баллы клиента в POS',
'Enable_disable_customer_points_system_in_POS' => 'Включить/Отключить систему бонусных баллов клиента в POS',
'Show_Categories_in_POS' => 'Показывать категории в POS',
'Show_hide_categories_in_POS' => 'Показать/Скрыть категории в POS',
'Show_Brands_in_POS' => 'Показывать бренды в POS',
'Show_hide_brands_in_POS' => 'Показать/Скрыть бренды в POS',
'Name_is_required' => 'Имя обязательно',
'Sellers' => 'Продавцы',
'AvgPerSeller' => 'Среднее на продавца',
'TopSeller' => 'Лучший продавец',
'SalesByPaymentMethod' => 'Продажи по способу оплаты',
'CurrentPage' => 'Текущая страница',
'Show_Logo' => 'Показывать логотип',
'Show_Store_Name' => 'Показывать название магазина',
'Show_Reference' => 'Показывать номер',
'Show_Date' => 'Показывать дату',
'Show_Seller' => 'Показывать продавца',
'Show_Paid_Line' => 'Показывать оплаченную строку',
'Show_Due_Line' => 'Показывать строку задолженности',
'Show_Payments_Table' => 'Показывать таблицу платежей',
'Show_ZATCA_QR' => 'Показывать QR ZATCA',
'Receipt_Paper_Size' => 'Размер бумаги чека',
'Paper_58mm' => '58 мм (малый)',
'Paper_80mm' => '80 мм (стандартный)',
'Paper_88mm' => '88 мм (широкий)',
'POS_receipt_layout' => 'Макет чека POS',
'POS_receipt_layout_default' => 'Макет чека POS по умолчанию',
'Layout_1_Standard' => 'Макет 1 — Стандартный',
'Layout_2_Compact' => 'Макет 2 — Компактный',
'Layout_3_Detailed' => 'Макет 3 — Подробный',
'Discount_from_Points' => 'Скидка за баллы',
'Show_Tax' => 'Показывать налог',
'Show_Discount' => 'Показывать скидку',
'Show_Shipping' => 'Показывать доставку',
'Logo_Size' => 'Размер логотипа',
'Logo_Size_Description' => 'Установите размер логотипа в пикселях (20–200px)',
'Small' => 'Малый',
'Medium' => 'Средний',
'Large' => 'Большой',
'Custom' => 'Пользовательский',
'Custom_Logo_Size' => 'Пользовательский размер логотипа',
'Receipt_Settings' => 'Настройки чека',
'View_All_Records' => 'Просмотреть все записи',
'Allow_user_to_view_all_records_description' => 'Разрешить пользователю просматривать все записи, а не только свои',
'User_Management' => 'Управление пользователями',
'Access_Warehouses' => 'Доступ к складам',
'Credit_Limit' => 'Кредитный лимит',
'No_limit' => 'Без лимита',
'Maximum_credit_amount_allowed_for_this_customer' => 'Максимальная сумма кредита для данного клиента',
'Maximum_credit_amount_allowed_for_this_customer_0_means_No_limit' => 'Максимальная сумма кредита для данного клиента (0 означает без лимита)',
'Previous_Dues' => 'Предыдущие задолженности',
'Opening_Balance_Previous_Dues' => 'Начальный баланс (предыдущие задолженности)',
'Opening_Balance_first' => 'Сначала начальный баланс',
'Payment_Allocation' => 'Распределение платежей',
'Payment_Allocation_description' => 'При оплате система сначала списывает начальный баланс, затем применяет остаток к задолженности по продажам',
'Credit_Limit_Exceeded' => 'Кредитный лимит превышен',
'exceeds_credit_limit_of' => 'превышает кредитный лимит',
'Sales_Due' => 'Задолженность по продажам',
'Current_Sales' => 'Текущие продажи',
'Total_Due' => 'Общая задолженность',
'Grand_Total' => 'Итого',
'Maximum_payment' => 'Максимальный платёж',
'Payment_Returns' => 'Возвраты платежей',
'CustomFields' => 'Пользовательские поля',
'CustomerCustomFields' => 'Пользовательские поля клиента',
'SupplierCustomFields' => 'Пользовательские поля поставщика',
'FieldName' => 'Название поля',
'FieldType' => 'Тип поля',
'Required' => 'Обязательно',
'Optional' => 'Необязательно',
'SelectOptions' => 'Параметры выбора',
'EnterOptionsOnePerLine' => 'Введите параметры, по одному на строку',
'DefaultValue' => 'Значение по умолчанию',
'SortOrder' => 'Порядок сортировки',
'Text' => 'Текст',
'Number' => 'Число',
'Textarea' => 'Текстовое поле',
'Date' => 'Дата',
'Select' => 'Выбрать',
'Checkbox' => 'Флажок',
'Quick_Add_Supplier' => 'Быстрое добавление поставщика',
'Weight' => 'Вес',
'Approve_Transfer' => 'Подтвердить перевод',
'Are_you_sure_you_want_to_approve_this_transfer' => 'Вы уверены, что хотите подтвердить этот перевод?',
'Approve' => 'Подтвердить',
'Transfer_approved_successfully' => 'Перевод успешно подтверждён',
'Failed_to_approve_transfer' => 'Не удалось подтвердить перевод',
'Transfer_Info' => 'Информация о переводе',
'Failed_to_load_transfer_details' => 'Не удалось загрузить детали перевода',
'Choose_Technician' => 'Выбрать техника',
'Import_Customers' => 'Импорт клиентов',
'Customers_without_Login' => 'Клиенты без входа',
'Customers_with_Login' => 'Клиенты с входом',
'Import_Suppliers' => 'Импорт поставщиков',
'Points' => 'Баллы',
'Item_Name' => 'Название позиции',
'Service_Job_Details' => 'Детали сервисного задания',
'Generating_PDF' => 'Создание PDF...',
'PDF_downloaded_successfully' => 'PDF успешно загружен',
'Back_to_Jobs' => 'Назад к заданиям',
'Job_not_found' => 'Сервисное задание не найдено.',
'pos.Loading_Application' => 'Загрузка приложения',
'pos.Loading_Application_help' => 'Пожалуйста, подождите, пока подготавливается интерфейс POS.',
'pos.Syncing_offline_sales' => 'Синхронизация офлайн-продаж',
'pos.Syncing_offline_sales_help' => 'Пожалуйста, подождите, пока офлайн-продажи синхронизируются.',
'pos.InternetRestored' => 'Интернет-соединение восстановлено',
'pos.ActiveCheckoutReloadQuestion' => 'У вас есть активная продажа. Обновить страницу сейчас или после завершения продажи?',
'pos.ReloadAfterSale' => 'После этой продажи',
'pos.ReloadNow' => 'Обновить сейчас',
'Dark_Mode_Updated' => 'Тёмный режим успешно обновлён',
'Failed_to_save_dark_mode' => 'Не удалось сохранить настройки тёмного режима',
'Configuration' => 'Конфигурация',
'Display_Price' => 'Отображать цену',
'Auto_Print' => 'Автопечать',
'Selected_Products' => 'Выбранные товары',
'Barcode_Preview' => 'Предпросмотр штрихкода',
'Pages' => 'Страницы',
'Invoice_Format' => 'Формат счета',
'Invoice_Thermal' => 'Термочек (POS)',
'Invoice_A4' => 'Счет A4 (PDF)',
'Invoice_Format_help' => 'Выберите способ печати счетов в POS: термочек или PDF A4.',
'Enable_Print_Invoice_help'  => 'Справка по включению печати счета',
'Sales_by_Payment' => 'Продажи по способу оплаты',
'Stock_Value' => 'Стоимость запасов',
'Stock_Value_by_Cost' => 'Стоимость запасов по себестоимости',
'Stock_Value_by_Retail' => 'Стоимость запасов по розничной цене',
'Stock_Value_by_Wholesale' => 'Стоимость запасов по оптовой цене',
'Due_Before' => 'Срок до',
'Due_After' => 'Срок после',
'Hide_From_Online_Store' => 'Скрыть из интернет-магазина',
'Security_Settings' => 'Настройки безопасности',
'System' => 'Система',
'Prefixes' => 'Префиксы',
'Login_Device_Management' => 'Управление устройствами входа',
'Date_Format' => 'Формат даты',
'Price_Format' => 'Формат цены',
'How_many_items_do_you_want_to_display_in_POS' => 'Сколько товаров отображать в POS',
'Analytics_Report' => 'Аналитический отчет',
'Stock_Inventory_Valuation' => 'Оценка складских запасов',
'Google_Calendar' => 'Google Календарь',
'Google_Calendar_Booking_Description' => 'Когда бронирование подтверждено, в вашем Google Календаре создаётся событие с датой/временем, услугой, данными клиента и напоминаниями. Введите ниже данные Google API и подключите свой аккаунт.',
'Booking_List' => 'Список бронирований',
'Connect_Google_Calendar' => 'Подключить Google Календарь',
'Disconnect' => 'Отключить',
'Connected' => 'Подключено',
'Not_connected' => 'Не подключено',
'Client_ID' => 'ID клиента',
'Client_Secret' => 'Секрет клиента',
'Redirect_URI' => 'URI перенаправления',
'Google_Calendar_Client_ID_Help' => 'Из Google Cloud Console → API и сервисы → Учетные данные.',
'Google_Calendar_Client_Secret_Help' => 'Оставьте пустым, чтобы сохранить существующий секрет.',
'Google_Calendar_Redirect_URI_Help' => 'Должен совпадать с URI, настроенным в Google Cloud Console. Оставьте пустым для использования по умолчанию.',
'Calendar_ID' => 'ID календаря',
'Leave_primary_for_default' => 'Оставьте "primary" для основного календаря.',
'App_default' => 'По умолчанию в приложении',
'Used_for_booking_events' => 'Используется для времени событий бронирования в Google Календаре.',
'Display' => 'Отображение',
'Screen' => 'Экран',
'Multiple_Displays' => 'Несколько экранов',
'Use_one_URL_per_screen_In_POS_select_matching_screen' => 'Используйте один URL для каждого физического экрана. В POS выберите соответствующий номер экрана, чтобы каждая касса управляла своим дисплеем.',
'Show_QR_Code' => 'Показать QR-код',
'Expenses_by_Category' => 'Расходы по категориям',
'Deposits_by_Category' => 'Депозиты по категориям',
'Distribution' => 'Распределение',
'Top_Category' => 'Лучшая категория',
'No_Data' => 'Нет данных за выбранный период',
'Selling_Price_Unit' => 'Цена продажи (Единица)',
'Current_Quantity' => 'Текущее количество',
'Stock_Value_Cost' => 'Стоимость запасов (Себестоимость)',
'Stock_Value_Selling' => 'Стоимость запасов (Продажа)',
'Potential_Profit' => 'Потенциальная прибыль',
'Total_Units_Sold' => 'Общее количество проданных единиц',
'Total_Units_Transferred' => 'Общее количество переданных единиц',
'Total_Units_Adjusted' => 'Общее количество скорректированных единиц',



'Receiver_Account_Number' => 'Номер счета получателя',
'Payment_Reference_Number' => 'Референсный номер платежа',
'Add_Other_Charges' => 'Добавить доставку и другие расходы',
'Charge_Name' => 'Название расхода',
'Additional_Fields' => 'Дополнительные поля (Вес / Текст / Число)',
'Field_Name' => 'Название поля',
'Field_Value' => 'Значение',
'Enable_Purchase_Extra_Charges' => 'Enable Extra Charges on Purchases',
'Enable_Purchase_Extra_Charges_Help' => 'When enabled, the purchase create/edit form shows the "Add More Shipping And Other Charge" section. Charges are added to the purchase Grand Total.',
'Enable_Purchase_Custom_Fields' => 'Enable Additional Fields on Purchases',
'Enable_Purchase_Custom_Fields_Help' => 'When enabled, the purchase create/edit form shows the "Additional Fields (Weight / Text / Number)" section. These fields are informational and do not affect totals.',
];
