  ...
  <div class="portal-login">
    ...
    <div class="pc-login-card">
      <div class="pc-login-brand">
        ...
        <p class="pc-sub">Inicia sesión para consultar tus facturas y pagos</p>
      </div>
      <form @submit.prevent="login" class="pc-form">
        <div class="pc-field">
          ...
          <div class="pc-input-wrap">
            ...
            <input v-model="email" type="email" required placeholder="correo@ejemplo.com" autocomplete="email" />
          </div>
        </div>
        <div class="pc-field">
  ...
  methods: {
    async login() {
      ...
      try {
        ...
        this.error = (data && data.message) || 'No se pudo iniciar sesión. Verifica tus datos e inténtalo de nuevo.';
      } finally {
        this.loading = false;
      }
    },
...
.pc-field label { font-size: 0.82rem; font-weight: 500; color: var(--pc-text); }
...
.pc-input-wrap input {
  width: 100%; padding: 0.7rem 1rem 0.7rem 2.35rem; border: 1px solid var(--pc-border-strong);
...
.pc-btn-spinner { display: inline-block; width: 1rem; height: 1rem; border: 2px solid rgba(255,255,255,0.35); border-top-color: #fff; border-radius: 50%; animation: pc-spin 0.7s linear infinite; }
