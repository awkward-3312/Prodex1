<template>
  <div class="px-next pxff">
    <px-page-header :title="$t('Create_Expense')" :breadcrumbs="[{ label: $t('Expenses'), href: '#/app/expenses/list' }, { label: $t('Create_Expense') }]" />

    <div v-if="isLoading" class="pxff__pad">
      <px-skeleton variant="lines" :rows="6" />
    </div>

    <validation-observer v-else ref="Create_Expense">
