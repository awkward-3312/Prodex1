  ...
  <div class="main-content">
    ...
    <b-card class="issues-card">
      <div class="issues-toolbar">
        ...
        <div class="d-flex align-items-center"><b-form-select v-model="status" :options="statusOptions" class="me-2" @change="load" /><b-button variant="outline-primary" :disabled="loading" @click="load">Actualizar</b-button></div>
      </div>
      ...
      <div v-else class="table-responsive">
        <table class="table table-hover">
          <thead><tr><th>Transferencia</th><th>Producto</th><th>Ruta</th><th>Tipo</th><th class="text-end">Cantidad</th><th>Estado</th><th>Reportado</th><th v-if="canManage">Acción</th></tr></thead>
          <tbody><tr v-for="issue in issues" :key="issue.id">
            ...
            <td class="text-end">{{ fmt(issue.quantity) }}</td>
    ...
    <b-modal v-model="showResolve" title="Resolver incidencia" hide-footer>
      <div v-if="selected">
        ...
        <div class="text-end"><b-button variant="secondary" class="me-2" @click="showResolve=false">Cancelar</b-button><b-button variant="primary" :disabled="saving || !canSubmit" @click="resolve">{{ saving ? 'Guardando…' : 'Resolver incidencia' }}</b-button></div>
      </div>
    </b-modal>
  </div>
</template>
