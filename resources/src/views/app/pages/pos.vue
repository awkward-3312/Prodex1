  ...
  <div class="pos-codecanyon px-next" data-pxn-type="plex" style="display: flex; flex-direction: column; height: 100vh; background: var(--pxn-bg); color: var(--pxn-ink); font-family: var(--pxn-font-sans); -webkit-font-smoothing: antialiased; overflow: hidden;">
    ...
    <nav v-if="productsReady" class="pos-shell-header" style="display: flex; align-items: center; gap: 8px; padding: 0 16px; flex-shrink: 0; flex-grow: 0; border-bottom: 1px solid var(--line); background: var(--surface);">
      ...
      <button
        ...
        style="width: 32px; height: 32px; border-radius: var(--pxn-radius-md); border: 1px solid var(--line); background: var(--surface); color: var(--ink-2); display: inline-flex; align-items: center; justify-content: center; padding: 0; cursor: pointer; transition: background-color 120ms var(--pxn-ease), border-color 120ms var(--pxn-ease);">
      ...
      <b-dropdown
        ...
        <div style="padding: 6px; min-width: 160px;">
          <button
            ...
            :style="{ display: 'block', width: '100%', textAlign: 'left', padding: '6px 10px', fontSize: '12px', borderRadius: '6px', background: customer_display_screen_id === opt.value ? 'var(--pxn-primary-soft)' : 'transparent', color: customer_display_screen_id === opt.value ? 'var(--pxn-primary-ink)' : 'var(--ink)', border: 0, cursor: 'pointer' }">
            {{ opt.label }}
          </button>
        </div>
      </b-dropdown>
      ...
      <div class="pos-shell-user-dd-wrapper" style="display: inline-flex;">
        <b-dropdown id="user-dd" right toggle-class="dropdown-toggle-no-caret p-0 bg-transparent border-0" no-caret variant="link">
          <template #button-content>
            ...
            <div v-else style="width: 32px; height: 32px; border-radius: 99px; background: var(--pxn-primary); color: var(--pxn-primary-contrast); display: inline-flex; align-items: center; justify-content: center; font-size: 13px; font-weight: 600;">{{ currentUser && currentUser.name ? currentUser.name.charAt(0).toUpperCase() : 'U' }}</div>
          </template>
    ...
    >
      ...
      >
        ...
        <div class="pos-shell-mobile-cart-header">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width: 16px; height: 16px; color: var(--ink-2);">
              ...
              <table class="table_data" style=" width: 100%; ">
                <tbody>
                  ...
                  <template v-if="invoicePromotions.length > 0">
                    <tr v-for="(promo, idx) in invoicePromotions" :key="'promo-a-' + idx + '-' + promo.id">
                      <td colspan="3" class="total">{{ $t('Promotions') || 'Promotions' }} — {{ promo.name }}<span v-if="promo.code"> ({{ promo.code }})</span></td>
                      <td style="text-align:right;" class="total">
                        −{{ formatPriceWithSymbol(invoice_pos.symbol, promo.amount, 2) }}
                      </td>
                    </tr>
                  </template>
                  <tr v-else-if="Number(invoice_pos.sale.promotion_discount || 0) > 0">
                    <td colspan="3" class="total">{{ $t('Promotions') || 'Promotions' }}<span v-if="invoice_pos.sale.promotion_code"> ({{ invoice_pos.sale.promotion_code }})</span></td>
                    <td style="text-align:right;" class="total">
                      −{{ formatPriceWithSymbol(invoice_pos.symbol, invoice_pos.sale.promotion_discount, 2) }}
                    </td>
                  </tr>
            ...
            <div v-else-if="currentReceiptLayout === 2">
              ...
              <table class="table_data mt-2" style="width:100%; font-size:11px;">
                <tbody>
                  ...
                  <template v-if="invoicePromotions.length > 0">
                    <tr v-for="(promo, idx) in invoicePromotions" :key="'promo-b-' + idx + '-' + promo.id">
                      <td class="total">{{ $t('Promotions') || 'Promotions' }} — {{ promo.name }}<span v-if="promo.code"> ({{ promo.code }})</span></td>
                      <td style="text-align:right" class="total">
                        −{{ formatPriceWithSymbol(invoice_pos.symbol, promo.amount, 2) }}
                      </td>
                    </tr>
                  ...
                  <tr v-else-if="Number(invoice_pos.sale.promotion_discount || 0) > 0">
                    <td class="total">{{ $t('Promotions') || 'Promotions' }}<span v-if="invoice_pos.sale.promotion_code"> ({{ invoice_pos.sale.promotion_code }})</span></td>
                    <td style="text-align:right" class="total">
                      −{{ formatPriceWithSymbol(invoice_pos.symbol, invoice_pos.sale.promotion_discount, 2) }}
                    </td>
                  </tr>
            ...
            <div v-else-if="currentReceiptLayout === 3">
              ...
              <table class="table_data w-100 mt-2" style="font-size:11px;">
                <tbody>
                  ...
                  <template v-if="invoicePromotions.length > 0">
                    <tr v-for="(promo, idx) in invoicePromotions" :key="'promo-c-' + idx + '-' + promo.id">
                      <td class="total">{{ $t('Promotions') || 'Promotions' }} — {{ promo.name }}<span v-if="promo.code"> ({{ promo.code }})</span></td>
                      <td style="text-align:right;" class="total">
                        −{{ formatPriceWithSymbol(invoice_pos.symbol, promo.amount, 2) }}
                      </td>
            ...
            <div v-else-if="currentReceiptLayout === 5" class="receipt-layout-5">
              ...
              <table class="minimal-totals">
                <tbody>
                  ...
                  <template v-if="invoicePromotions.length > 0">
                    <tr v-for="(promo, idx) in invoicePromotions" :key="'promo-e-' + idx + '-' + promo.id">
                      <td>{{ $t('Promotions') || 'Promotions' }} — {{ promo.name }}<span v-if="promo.code"> ({{ promo.code }})</span></td>
                      <td>−{{ formatPriceWithSymbol(invoice_pos.symbol, promo.amount, 2) }}</td>
                    </tr>
                  </template>
                  <tr v-else-if="Number(invoice_pos.sale.promotion_discount || 0) > 0">
                    <td>{{ $t('Promotions') || 'Promotions' }}<span v-if="invoice_pos.sale.promotion_code"> ({{ invoice_pos.sale.promotion_code }})</span></td>
...
export default {
  ...
  data() {
    return {
      ...
      // Promotions (auto-applied at checkout based on cart context).
      promotionDiscount: 0,
      appliedPromotions: [],
      promotionCode: "",
  ...
  computed: {
    ...
    // Per-promotion lines for the receipt (one row per applied promotion).
    // Falls back to an empty array for old sales that have no breakdown.
    invoicePromotions() {
      const sale = this.invoice_pos && this.invoice_pos.sale;
      if (!sale || !Array.isArray(sale.promotions)) return [];
      return sale.promotions.filter(p => Number(p && p.amount) > 0);
    },

    // Forwarded to ModernPaymentModal so it can include the same promotion
    // fields the legacy Submit_Payment path sent — otherwise the backend
    // never sees a promotion context and no PromotionUsage rows are created.
    promotionContextForCheckout() {
      return {
  ...
  watch: {
    ...
    'sale.warehouse_id'(newVal, oldVal) {
      // Promotions are warehouse-scoped, so re-evaluate on every change.
      this.Schedule_Refresh_Promotions && this.Schedule_Refresh_Promotions();
    ...
    // Per-customer promotion caps depend on the selected client, so re-evaluate
    // promotions when the cashier changes the customer on an active cart.
  ...
  methods: {
    ...
    CalculTotal() {
      ...
      // Subtract auto-applied promotions on top of the manual discount, capped to
      // the cart subtotal so the line can never go negative.
      const promoDiscount = Math.min(Number(this.promotionDiscount || 0), Math.max(this.total - discountAmount, 0));
      const total_without_discount = parseFloat(
        (this.total - discountAmount - promoDiscount).toFixed(this.priceDecimals)
      );
    ...
    // Re-fetch applicable promotions when the cart changes — unless this
    // CalculTotal was itself triggered by a fresh promotion result (avoids
    // an extra round-trip per user action).
    if (!this.CalculTotal_skipPromoRefresh) {
      this.Schedule_Refresh_Promotions();
    }
    },

    // ----------------- Promotions (auto-applied) -----------------

    buildPromotionProductSubtotals() {
      // Per-line subtotals so the backend can scope specific-product promotions
      // to the eligible lines instead of discounting the full cart total.
      const out = [];
      if (!this.details || !this.details.length) return out;
      for (let i = 0; i < this.details.length; i++) {
        ...
        const lineSubtotal = Number(
          d.subtotal != null
            ? d.subtotal
            : ((Number(d.Net_price) || 0) + (Number(d.taxe) || 0)) * (Number(d.quantity) || 0)
        ) || 0;
        out.push({ product_id: Number(d.product_id), subtotal: lineSubtotal });
      }
      return out;
    },

    Schedule_Refresh_Promotions() {
      if (this._promotionRefreshTimer) {
        clearTimeout(this._promotionRefreshTimer);
      }
      this._promotionRefreshTimer = setTimeout(() => {
        this.Refresh_Promotions();
      }, 250);
    },

    Refresh_Promotions() {
      // No warehouse / empty cart → nothing to evaluate.
      if (!this.sale.warehouse_id || !this.details || this.details.length === 0) {
        if (this.promotionDiscount !== 0 || this.appliedPromotions.length > 0) {
          this.promotionDiscount = 0;
          this.appliedPromotions = [];
          this.$nextTick(() => this.CalculTotal && this.CalculTotal());
        }
        return;
      }
      ...
      for (let i = 0; i < this.details.length; i++) {
        const d = this.details[i];
        if (d && d.product_id) productIds.push(Number(d.product_id));
        itemCount += Number(d.quantity || 0);
      }

      const payload = {
        warehouse_id: this.sale.warehouse_id,
        subtotal: Number(this.total || 0),
        item_count: itemCount,
        product_ids: productIds,
        product_subtotals: this.buildPromotionProductSubtotals(),
        code: this.promotionCode || null,
        client_id: this.selectedClientId || null,
      };

      axios
        .post("promotions/applicable", payload)
        .then(response => {
          const data = response && response.data ? response.data : {};
          const newDiscount = Number(data.total_discount || 0);
          const newApplied = Array.isArray(data.applied) ? data.applied : [];
          // Avoid infinite recompute loops: only fire CalculTotal when the value
          // actually changed.
          const changed = newDiscount !== this.promotionDiscount
            || newApplied.length !== this.appliedPromotions.length;
          this.promotionDiscount = newDiscount;
          this.appliedPromotions = newApplied;
          if (changed) {
            this.$nextTick(() => {
              // Recompute totals without re-triggering the watcher loop.
              this.CalculTotal_skipPromoRefresh = true;
              this.CalculTotal();
              this.CalculTotal_skipPromoRefresh = false;
            });
          }
        })
        .catch(() => {
          // Silent fail: promotions are optional, the cashier can still complete the sale.
        });
    },
    ...
    Apply_Promotion_Code() {
      ...
      if (!this.sale.warehouse_id) {
        this.promotionCodeStatus = "error";
        this.promotionCodeMessage = this.$t("Select_Warehouse") || "Select a warehouse first.";
        return;
      }
      axios
        .post("promotions/validate_code", {
          code: code,
          warehouse_id: this.sale.warehouse_id,
          client_id: this.selectedClientId || null,
        })
        .then(response => {
          const data = response && response.data ? response.data : {};
          if (data.ok) {
            this.promotionCodeStatus = "ok";
            this.promotionCodeMessage = (data.promotion && data.promotion.name) || (this.$t("CodeApplied") || "Code applied");
            this.Refresh_Promotions();
          } else {
            this.promotionCodeStatus = "error";
            this.promotionCodeMessage = data.reason || (this.$t("InvalidCode") || "Invalid code");
          }
        })
        .catch(() => {
          this.promotionCodeStatus = "error";
          this.promotionCodeMessage = this.$t("InvalidCode") || "Invalid code";
        });
    },
...
.ps-switch-slider {
  ...
  border-radius: var(--pxn-radius-pill);
  cursor: pointer;
  transition: background var(--pxn-dur-2) var(--pxn-ease), border-color var(--pxn-dur-2) var(--pxn-ease);
}
...
/* Fields */
.ps-field { margin-top: var(--pxn-space-5); }
.ps-field-label {
  display: block;
  font-size: var(--pxn-fs-sm);
  font-weight: var(--pxn-fw-medium);
...
/* ============================================
   ...
   ============================================ */
/* ==========================================================================
   POS topbar · one operational-selector family
   Warehouse/Location · Category · Brand · Customer share the same flat
   px-next chrome: neutral icon tile + optional eyebrow + value + caret,
   hairline border, token hover, no gradients / no glow / no motion.
   ========================================================================== */
.pos-cat-trigger {
  display: inline-flex;
  align-items: center;
  gap: var(--pxn-space-4);
  height: var(--pxn-control-h-sm);
  padding: 0 var(--pxn-space-4) 0 var(--pxn-space-4);
  min-width: 150px;
  max-width: 200px;
  border-radius: var(--pxn-radius-md);
  border: 1px solid var(--line);
  ...
  .pos-cat-trigger-clear {
    ...
    justify-content: center;
    cursor: pointer;
    transition: background-color var(--pxn-dur-1) var(--pxn-ease), color var(--pxn-dur-1) var(--pxn-ease);
    flex: 0 0 auto;
    padding: 0;
    i { font-size: 12px; }
  ...
  &.is-active {
    border-color: var(--pxn-selected-border);
    background: var(--pxn-selected-bg);
    .pos-cat-trigger-icon { background: var(--pxn-primary-soft); color: var(--pxn-primary-ink); }
    .pos-cat-trigger-label { color: var(--pxn-primary-ink); font-weight: var(--pxn-fw-semibold); }
  }
}
...
.cat-drawer {
  ...
  border-top-right-radius: var(--pxn-radius-xl);
  border-bottom-right-radius: var(--pxn-radius-xl);
  overflow: hidden;
}
...
/* Header */
.cat-drawer-header {
  padding: var(--pxn-space-6) var(--pxn-space-6) var(--pxn-space-5);
  display: flex;
  align-items: center;
  justify-content: space-between;
  gap: var(--pxn-space-5);
...
.cat-drawer-list {
  flex: 1 1 auto;
  overflow-y: auto;
  padding: var(--pxn-space-3) var(--pxn-space-5) var(--pxn-space-6);
  display: flex;
  flex-direction: column;
  gap: var(--pxn-space-2);
}
.cat-drawer-item {
  ...
  &.active {
    background: var(--pxn-selected-bg);
    border-color: var(--pxn-selected-border);
    .cat-drawer-item-name { color: var(--pxn-primary-ink); font-weight: var(--pxn-fw-semibold); }
  }
}
...
.cat-drawer-empty {
  text-align: center;
  padding: var(--pxn-space-9) var(--pxn-space-5) var(--pxn-space-7);
  color: var(--ink-3);
}
...
/* ============================================
   ...
   ============================================ */
/* Warehouse / operational location — same family, two-line body (eyebrow + value). */
.pos-wh-trigger {
  display: inline-flex;
  align-items: center;
  gap: var(--pxn-space-4);
  ...
  &.is-active {
    border-color: var(--pxn-selected-border);
    background: var(--pxn-selected-bg);
    .pos-wh-trigger-icon { background: var(--pxn-primary-soft); color: var(--pxn-primary-ink); }
  }
}
...
.wh-drawer {
  ...
  flex-direction: column;
  box-shadow: var(--pxn-shadow-modal);
  border-top-left-radius: var(--pxn-radius-xl);
  border-bottom-left-radius: var(--pxn-radius-xl);
  overflow: hidden;
}
...
.wh-drawer-header {
  position: relative;
  padding: var(--pxn-space-7) var(--pxn-space-7) var(--pxn-space-6);
  background: var(--surface);
  border-bottom: 1px solid var(--line);
  color: var(--ink);
}
...
.wh-drawer-card {
  ...
  &.active {
    border-color: var(--pxn-selected-border);
    background: var(--pxn-selected-bg);
    .wh-drawer-card-icon { background: var(--pxn-primary-soft); color: var(--pxn-primary-ink); }
    .wh-drawer-card-name { color: var(--pxn-primary-ink); }
  }
}
.wh-drawer-card-icon {
  width: 40px;
...
/* ============================================
   ...
   ============================================ */
/* Customer — same family, slightly higher emphasis: accent avatar + firmer
   active border. Round avatar (it's a person); the template sets a per-client
   colour inline, this is the walk-in fallback. */
.pos-cust-trigger {
  display: inline-flex;
  align-items: center;
  gap: var(--pxn-space-4);
...
.cust-drawer-header {
  position: relative;
  padding: var(--pxn-space-7) 60px var(--pxn-space-6) var(--pxn-space-7);
  display: flex;
  align-items: center;
  gap: var(--pxn-space-6);
  background: var(--surface);
...
.cust-drawer-quick-add {
  ...
  align-items: center;
  gap: var(--pxn-space-3);
  padding: var(--pxn-space-3) var(--pxn-space-5);
  border-radius: var(--pxn-radius-md);
  border: 1px solid var(--pxn-border-control);
  background: var(--surface);
  color: var(--ink);
...
.cust-drawer-list {
  flex: 1 1 auto;
  overflow-y: auto;
  padding: var(--pxn-space-4) var(--pxn-space-5) var(--pxn-space-6);
  display: flex;
  flex-direction: column;
  gap: var(--pxn-space-2);
}
.cust-drawer-card {
  ...
  &.active {
    background: var(--pxn-selected-bg);
    border-color: var(--pxn-selected-border);
    .cust-drawer-card-name { color: var(--pxn-primary-ink); }
  }
}
...
.cust-drawer-card-clear {
  ...
  &.active {
    background: var(--soft);
    border-color: var(--line-strong);
    .cust-drawer-card-name { color: var(--ink); }
...
.cust-drawer-empty {
  text-align: center;
  padding: var(--pxn-space-9) var(--pxn-space-6) var(--pxn-space-8);
  color: var(--ink-3);
}
...
@media (max-width: 768px) {
  ...
  .pos-codecanyon .pos-shell-cart-row > div:first-child {
    ...
    background-color: var(--pxn-surface-2) !important;
    color: var(--ink-2) !important;
  }
