...
export default {
  ...
  data() {
    return {
      ...
      // MS6-B2 — the location active the last time Selected_Inventory_Location
      // ran; null until the very first resolution (initial load), so the
      // prefilled serial_numbers survive mount and are only cleared on an
      // actual user-driven location CHANGE.
      previousInventoryLocationId: null,
      total: 0,
      GrandTotal: 0,
    };
  },
  ...
  methods: {
    ...
    Selected_Inventory_Location(locationId) {
      ...
      // MS6-B2 — a serial selected/prefilled for a DIFFERENT previous location
      // is never valid at the new one: never conserve it silently. Skip the
      // very first resolution (initial load) so the prefilled serial_numbers
      // survive mount; only an actual user-driven CHANGE clears them.
      ...
      this.previousInventoryLocationId = id;
