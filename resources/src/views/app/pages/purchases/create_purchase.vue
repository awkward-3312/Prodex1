<template>
  <div class="px-next pxpuf">
    <px-page-header
      :title="$t('AddPurchase')"
      :breadcrumbs="[{ label: $t('Purchases') }, { label: $t('ListPurchases') }, { label: $t('AddPurchase') }]"
    >
      <template #actions>
              ...
              <template #default="{ id }">
                <div class="pxpuf__inline">
                  ...
                  <px-button
                    ...
                    variant="secondary" icon-only icon="plus" :title="$t('Quick_Add_Supplier')" @click="Quick_Add_Supplier" />
                </div>
              </template>
            </px-field>
          </validation-provider>
            ...
            <table class="pxpuf-tbl">
              ...
              <tbody>
                ...
                <template v-for="detail in details">
                  <tr :key="'detail-' + detail.detail_id">
                    ...
                    <td class="is-center">
                      <span class="pxpuf__stepper">
                        <button type="button" class="pxpuf__step" @click="decrement(detail, detail.detail_id)">−</button>
                        <input class="pxpuf__stepinput" @keyup="Verified_Qty(detail, detail.detail_id)" :min="0.00" v-model.number="detail.quantity" />
                        <button type="button" class="pxpuf__step" @click="increment(detail, detail.detail_id)">+</button>
                      </span>
                    </td>
                    ...
                    <td class="is-center pxpuf__rowactions">
                  ...
                  <tr v-if="detail.is_batch_tracked && purchase.statut !== 'received'" :key="'batches-defer-' + detail.detail_id" class="pxpuf__batchrow">
                    <td colspan="9">
                  ...
                  <!-- Batch entry -->
                  <tr v-if="detail.is_batch_tracked && purchase.statut === 'received'" :key="'batches-' + detail.detail_id" class="pxpuf__batchrow">
                    <td colspan="9">
    ...
    <px-modal v-model="quickAddSupplierOpen" :title="$t('Quick_Add_Supplier')" size="lg">
      <validation-observer ref="Quick_Add_Supplier_Form" tag="div">
        <div class="pxpuf__grid2">
          <validation-provider name="Name Supplier" :rules="{ required: true }" v-slot="v">
            <px-field :label="$t('SupplierName')" required :error="v.errors[0]">
              <template #default="{ id }"><px-input :id="id" v-model="supplier.name" :placeholder="$t('SupplierName')" @input="v.validate" /></template>
            </px-field>
          </validation-provider>
          <px-field :label="$t('Email')">
            <template #default="{ id }"><px-input :id="id" v-model="supplier.email" :placeholder="$t('Email')" /></template>
          </px-field>
          <px-field :label="$t('Phone')">
            <template #default="{ id }"><px-input :id="id" v-model="supplier.phone" :placeholder="$t('Phone')" /></template>
          </px-field>
          <px-field :label="$t('Country')">
            <template #default="{ id }"><px-input :id="id" v-model="supplier.country" :placeholder="$t('Country')" /></template>
          </px-field>
          <px-field :label="$t('City')">
            <template #default="{ id }"><px-input :id="id" v-model="supplier.city" :placeholder="$t('City')" /></template>
          </px-field>
          <px-field :label="$t('Tax_Number')">
            <template #default="{ id }"><px-input :id="id" v-model="supplier.tax_number" :placeholder="$t('Tax_Number')" /></template>
          </px-field>
          <px-field :label="$t('Adress')" class="pxpuf__grid2-full">
            <template #default="{ id }"><px-textarea :id="id" v-model="supplier.adresse" :rows="4" :placeholder="$t('Adress')" /></template>
          </px-field>
        </div>
      </validation-observer>
      <template #footer="{ close }">
        <px-button variant="secondary" @click="close">{{ $t('Cancel') }}</px-button>
        <px-button variant="primary" icon="check" :loading="SubmitProcessing" :disabled="SubmitProcessing" @click="Submit_Quick_Add_Supplier">{{ $t('submit') }}</px-button>
      </template>
    </px-modal>
...
export default {
  ...
  methods: {
    ...
    Quick_Add_Supplier() {
      ...
      this.quickAddSupplierOpen = true;
    },
        ...
        this.$refs.Quick_Add_Supplier_Form.validate().then(success => {
          ...
          axios
            ...
            .then(({ data }) => {
              ...
              this.quickAddSupplierOpen = false;
              this.reset_Form_supplier();
            })
            .catch(error => {
              NProgress.done();
