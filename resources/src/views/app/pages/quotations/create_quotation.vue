<template>
  <div class="px-next pxqf">
    <px-page-header
      :title="$t('AddQuote')"
      :breadcrumbs="[{ label: $t('Sales') }, { label: $t('ListQuotations') }, { label: $t('AddQuote') }]"
    >
      <template #actions>
    ...
    <validation-observer v-else ref="create_quote" tag="div">
...
export default {
  ...
  methods: {
    ...
    //--- vee-validate 3.x: seed prefilled/fetched values into providers so an
    //    untouched-but-valid field does not block submit (scopedSlot regression).
    syncValidators() {
      this.$nextTick(() => {
