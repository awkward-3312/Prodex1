...
<script>
import { vBTooltip, BModal, BButton, BCard, BForm, BFormGroup, BFormInput, BFormTextarea, BFormFile } from "@/platform/bootstrap";
import { modals, notifications } from "@/platform";
import NProgress from "nprogress";
export default { components: { BFormFile, BForm, BFormGroup, BFormInput, BFormTextarea, BButton, BCard, BModal }, directives: { 'b-tooltip': vBTooltip },
  ...
  methods: {
    ...
    onImageChange(e) { const f = e.target.files[0]; if (!f) return; this.imageFile = f; this.imagePreview = URL.createObjectURL(f); },
    New_Category() { this.editMode = false; this.form = { id: null, name: "", description: "" }; this.imageFile = null; this.imagePreview = null; modals.show("categoryModal"); },
    Edit_Category(row) { this.editMode = true; this.form = { id: row.id, name: row.name, description: row.description || "" }; this.imageFile = null; this.imagePreview = row.image ? "/" + row.image : null; modals.show("categoryModal"); },
    Get_Categories(page) { this.serverParams.page = page || 1; NProgress.start(); NProgress.set(0.1); axios.get("realestate/categories?page=" + this.serverParams.page + "&SortField=" + this.serverParams.sort.field + "&SortType=" + this.serverParams.sort.type + "&search=" + this.search + "&limit=" + this.limit).then(res => { this.totalRows = res.data.totalRows; this.categories = res.data.categories; NProgress.done(); this.isLoading = false; }).catch(() => { NProgress.done(); setTimeout(() => { this.isLoading = false; }, 500); }); },
    save() { if (!this.form.name) { this.makeToast("danger", this.$t("InvalidData"), this.$t("Failed")); return; } this.saving = true; const fd = new FormData(); fd.append("name", this.form.name); if (this.form.description) fd.append("description", this.form.description); if (this.imageFile) fd.append("image_file", this.imageFile); const url = this.editMode ? "realestate/categories/" + this.form.id : "realestate/categories"; axios.post(url, fd, { headers: { "Content-Type": "multipart/form-data" } }).then(() => { this.saving = false; modals.hide("categoryModal"); this.makeToast("success", this.$t("Created_in_successfully"), this.$t("Success")); this.Get_Categories(this.serverParams.page); }).catch(() => { this.saving = false; this.makeToast("danger", this.$t("InvalidData"), this.$t("Failed")); }); },
    Remove_Category(id) { this.$swal({ title: this.$t("Delete_Title"), text: this.$t("Delete_Text"), type: "warning", showCancelButton: true, confirmButtonColor: "var(--px-primary)", cancelButtonColor: "#d33", cancelButtonText: this.$t("Delete_cancelButtonText"), confirmButtonText: this.$t("Delete_confirmButtonText") }).then(result => { if (result.value || result.isConfirmed) { axios.delete("realestate/categories/" + id).then(() => { this.$swal(this.$t("Delete_Deleted"), this.$t("Deleted_in_successfully"), "success"); this.Get_Categories(this.serverParams.page); }).catch(() => this.$swal(this.$t("Delete_Failed"), this.$t("Delete_Therewassomethingwronge"), "warning")); } }); }
  },
