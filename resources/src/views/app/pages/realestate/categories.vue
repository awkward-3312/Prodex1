...
export default {
  ...
  methods: {
    ...
    New_Category() { this.editMode = false; this.form = { id: null, name: "", description: "" }; this.imageFile = null; this.imagePreview = null; this.$bvModal.show("categoryModal"); },
    Edit_Category(row) { this.editMode = true; this.form = { id: row.id, name: row.name, description: row.description || "" }; this.imageFile = null; this.imagePreview = row.image ? "/" + row.image : null; this.$bvModal.show("categoryModal"); },
    ...
    save() { if (!this.form.name) { this.makeToast("danger", this.$t("InvalidData"), this.$t("Failed")); return; } this.saving = true; const fd = new FormData(); fd.append("name", this.form.name); if (this.form.description) fd.append("description", this.form.description); if (this.imageFile) fd.append("image_file", this.imageFile); const url = this.editMode ? "realestate/categories/" + this.form.id : "realestate/categories"; axios.post(url, fd, { headers: { "Content-Type": "multipart/form-data" } }).then(() => { this.saving = false; this.$bvModal.hide("categoryModal"); this.makeToast("success", this.$t("Created_in_successfully"), this.$t("Success")); this.Get_Categories(this.serverParams.page); }).catch(() => { this.saving = false; this.makeToast("danger", this.$t("InvalidData"), this.$t("Failed")); }); },
