<template>
  <div class="px-next pxrl">
    <px-page-header :title="$t('Deposits_Report')" :breadcrumbs="[{ label: $t('Reports'), href: '#/app/reports/all' }, { label: $t('Deposits_Report') }]">
      <template #actions>
    ...
    <template v-else>
      <div class="pxrl__kpis pxrl__kpis--3">
        <px-stat bordered icon="landmark" :label="$t('Total_Deposits')" :value="formatPriceWithSymbol(currentUser && currentUser.currency, totalDeposits, 2)" />
        <px-stat bordered icon="bar-chart" :label="$t('Deposit_Category')" :value="String(categoryCount)" />
      ...
      <div class="pxrl__cols pxrl__gap">
        <px-card :title="`${$t('Deposits_by_Category') || 'Deposits by Category'} (${$t('Distribution') || 'Distribution'})`">
        ...
        <px-card :title="`${$t('Deposits_by_Category') || 'Deposits by Category'} (${$t('Total_Deposits')})`">
      ...
      <div class="pxrl__tablewrap">
        ...
        >
          <template #cell-total_deposits="{ row }"><span class="pxn-num">{{ formatPriceWithSymbol(currentUser && currentUser.currency, row.total_deposits, 2) }}</span></template>
        </px-table>

        <px-empty-state v-else icon="landmark" :title="$t('No_report_rows') || 'Sin resultados'" :description="$t('No_report_rows_desc')" />
      </div>

      <div v-if="reports.length" class="pxrl__totalrow">
        <span>{{ $t('Total') }}</span>
        <span class="pxn-num">{{ formatPriceWithSymbol(currentUser && currentUser.currency, totalDeposits, 2) }}</span>
      </div>
...
export default {
  ...
  computed: {
    ...
    columns() {
      return [
        { key: "category_name", label: this.$t("Deposit_Category"), strong: true },
        { key: "total_deposits", label: this.$t("Total_Deposits"), align: "right" },
      ];
    },
    totalDeposits() {
  ...
  methods: {
    onExport(item) {
      ...
      else if (k === "pdf") this.Deposits_report_pdf();
    ...
    exportCsv() {
      ...
      link.setAttribute("download", "Deposits_report.csv");
    ...
    onSearchInput(v) {
      ...
      this._searchTimer = setTimeout(() => { this.updateParams({ page: 1 }); this.get_deposits_report(1); }, 350);
    },
    ...
    onPage(p) { if (this.serverParams.page !== p) { this.updateParams({ page: p }); this.get_deposits_report(p); } },

    onLimit(v) { if (this.limit !== String(v)) { this.limit = String(v); this.updateParams({ page: 1, perPage: Number(v) }); this.get_deposits_report(1); } },

    sumCount(rowObj) {
      let sum = 0;
      for (let i = 0; i < rowObj.children.length; i++) {
        sum += parseFloat(rowObj.children[i].total_deposits) || 0;
      }
      return sum;
    },
    ...
    printTableOnly() {
      ...
      reports.forEach(report => {
        ...
        this.columns.forEach(col => {
          ...
          } else if (col.key === 'total_deposits') {
