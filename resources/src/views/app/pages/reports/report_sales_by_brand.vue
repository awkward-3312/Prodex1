<template>
  <div class="px-next pxrl">
    <px-page-header :title="$t('Sales_by_Brand')" :breadcrumbs="[{ label: $t('Reports'), href: '#/app/reports/all' }, { label: $t('Sales_by_Brand') }]">
      <template #actions>
...
export default {
  ...
  computed: {
    ...
    columns() {
      return [
        { key: "brand_name", label: this.$t("Brand"), strong: true },
  ...
  methods: {
    ...
    onSearchInput(v) {
      ...
      this._searchTimer = setTimeout(() => { this.updateParams({ page: 1 }); this.get_sales_by_brand(1); }, 350);
    },

    onPage(p) { if (this.serverParams.page !== p) { this.updateParams({ page: p }); this.get_sales_by_brand(p); } },

    onLimit(v) { if (this.limit !== String(v)) { this.limit = String(v); this.updateParams({ page: 1, perPage: Number(v) }); this.get_sales_by_brand(1); } },
    ...
    exportCsv() {
      ...
      link.setAttribute("download", "sales_by_brand_report.csv");
