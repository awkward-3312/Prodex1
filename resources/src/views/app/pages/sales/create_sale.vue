          ...
          <b-col lg="12" md="12" sm="12">
            <b-card>
              <b-row>
                  ...
                  <validation-provider name="inventory_location" :rules="{ required: true }">
                    <b-form-group slot-scope="{ valid, errors }" :label="$t('Inventory_Location') + ' *'">
                      <v-select
                        :class="{'is-invalid': !!errors.length}"
                        :state="errors[0] ? false : (valid ? true : null)"
                        :disabled="details.length > 0 || !sale.warehouse_id"
                ...
                <!-- Promo code -->
                <b-col lg="4" md="4" sm="12" class="mb-3" v-if="currentUserPermissions && currentUserPermissions.includes('edit_tax_discount_shipping_sale')">
                  <b-form-group :label="$t('PromoCode') || 'Promo code'">
                    <div class="d-flex" style="gap:8px; align-items:center;">
                      <b-form-input
                        ...
                        class="flex-grow-1"
                      ></b-form-input>
                      <b-button variant="primary" size="sm" @click="Apply_Promotion_Code">
                        {{ $t('Apply') || 'Apply' }}
                      </b-button>
                      <b-button v-if="promotionCode" variant="outline-secondary" size="sm" @click="Clear_Promotion_Code">×</b-button>
                    </div>
                    <small
                      v-if="promotionCodeStatus"
                      :class="promotionCodeStatus === 'ok' ? 'text-success' : 'text-danger'"
                      class="d-block mt-1"
                    >{{ promotionCodeMessage }}</small>
                    <small
                      v-if="Number(promotionDiscount) > 0"
                      class="text-success d-block mt-1"
                    >
                      ✅ {{ $t('Promotions') || 'Promotions' }}<span v-if="appliedPromotions.length"> ({{ appliedPromotions.map(p => p.name).join(', ') }})</span>: −{{ formatNumber(promotionDiscount, 2) }} {{ currentUser.currency }}
                    </small>
                  </b-form-group>
                </b-col>
...
export default {
  ...
  data() {
    return {
      ...
      total: 0,
      GrandTotal: 0,
      // Promotions (auto-applied based on cart context)
      promotionDiscount: 0,
      appliedPromotions: [],
      promotionCode: "",
      ...
      CalculTotal_skipPromoRefresh: false,
      units:[],
      product: {
        id: "",
        product_type: "",
  ...
  watch: {
    ...
    // Promotions are warehouse-scoped; re-evaluate on change.
    'sale.warehouse_id'(newVal, oldVal) {
      if (newVal !== oldVal) {
        this.Schedule_Refresh_Promotions && this.Schedule_Refresh_Promotions();
      }
    },
    // Per-customer caps depend on the selected client.
  ...
  methods: {
    ...
    // Serial / IMEI: true when a serialized line's selected serial count != required.
    ...
    CalculTotal() {
      ...
      // the remaining cart subtotal so the line can never go negative.
      const promoDiscount = Math.min(Number(this.promotionDiscount || 0), Math.max(this.total - discountAmount, 0));
      const total_without_discount = parseFloat(
        (this.total - discountAmount - promoDiscount).toFixed(2)
      );
      ...
      // CalculTotal was itself triggered by a fresh promotion result.
    ...
    Refresh_Promotions() {
      ...
      if (!this.sale.warehouse_id || !this.details || this.details.length === 0) {
        if (this.promotionDiscount !== 0 || this.appliedPromotions.length > 0) {
          ...
          this.$nextTick(() => {
            ...
            this.CalculTotal && this.CalculTotal();
      ...
      axios
        ...
        .catch(() => {
          // Silent fail: promotions are optional, the sale can still be created.
        });
    },
