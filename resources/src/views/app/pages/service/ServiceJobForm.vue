...
<script>
import { BModal, BBadge, BButton, BCol, BRow, BTab, BTabs, BForm, BFormGroup, BFormInvalidFeedback, BFormInput, BFormSelect, BFormCheckboxGroup, BFormTextarea, BInputGroup, BFormFile } from "@/platform/bootstrap";
...
export default { components: { BForm, BFormGroup, BFormInvalidFeedback, BFormInput, BFormSelect, BFormCheckboxGroup, BFormTextarea, BInputGroup, BFormFile, BBadge, BButton, BCol, BRow, BTab, BTabs, BModal },
  ...
  methods: {
    ...
    async Submit_Service_Job() {
      ...
      try {
        if (this.isEdit) {
          const { data } = await axios.put(`service_jobs/${this.jobId}`, payload);
          this.makeToast('success', this.$t('Successfully_Updated'), this.$t('Success'));
          this.showStockWarnings(data);
          await this.loadJobIfNeeded();
        } else {
    ...
    async markDelivered() {
      try {
        const { data } = await axios.post(`service_jobs/${this.jobId}/mark_delivered`);
        this.makeToast('success', this.$t('Successfully_Updated'), this.$t('Success'));
        this.showStockWarnings(data);
        await this.loadJobIfNeeded();
      } catch (e) {
    ...
    showStockWarnings(data) {
      ...
      warnings.forEach(msg => {
        this.makeToast('warning', msg, this.$t('Warning') || 'Warning');
      });
    },
