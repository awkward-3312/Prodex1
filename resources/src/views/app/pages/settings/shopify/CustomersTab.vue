  ...
  <div>
    ...
    <b-card class="sync-card shadow-sm mb-4">
      <template #header>
        <div class="d-flex align-items-center">
          ...
          <h5 class="mb-0 fw-bold">{{ $t('Push_customers_to_Shopify') }}</h5>
        </div>
      </template>
    ...
    <b-card class="sync-card shadow-sm mb-4">
      <template #header>
        <div class="d-flex align-items-center">
          ...
          <h5 class="mb-0 fw-bold">{{ $t('Pull_customers_from_Shopify') }}</h5>
        </div>
      </template>
    ...
    <b-card v-if="running || hasCounters" class="sync-card shadow-sm">
      ...
      <div v-if="batchErrors.length" class="mt-2">
        <b-alert :model-value="true" variant="warning" class="mb-0">
...
<script>
import { BRow, BCol, BCard, BButton, BBadge, BAlert } from "@/platform/bootstrap";
import syncRunner from './syncRunner';

export default {
  components: { BRow, BCol, BCard, BButton, BBadge, BAlert },
  mixins: [syncRunner],
