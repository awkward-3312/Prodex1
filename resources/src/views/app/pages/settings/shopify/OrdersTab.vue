    ...
    <b-card class="sync-card shadow-sm mb-4">
      <template #header>
        <div class="d-flex align-items-center">
          <lucide-icon class="me-2 text-primary" name="download" />
          <h5 class="mb-0 fw-bold">{{ $t('Import_orders_from_Shopify') }}</h5>
        </div>
      </template>
      ...
      <b-button variant="primary" :disabled="running" @click="pull">
        <lucide-icon class="me-1" name="download" /> {{ $t('Import_orders_now') }}
      </b-button>
