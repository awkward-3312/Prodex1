<template>
  <div class="px-next pxcfg">
    ...
    <template v-else>
      ...
      <div class="pxcfg__tablewrap">
        <px-table
          ...
          @sort="onSort"
        >
          <template #cell-webhook="{ row }">{{ row.webhook ? row.webhook.name : '—' }}</template>
          <template #cell-status="{ row }">
            <px-badge :tone="statusTone(row.status)">{{ row.status }}</px-badge>
          </template>
          <template #cell-response_code="{ row }">
            <px-badge v-if="row.response_code" :tone="httpTone(row.response_code)">{{ row.response_code }}</px-badge>
            <span v-else>—</span>
          </template>
        ...
        <px-empty-state v-else icon="file-text" title="Sin entregas" description="Aún no hay registros de entrega de webhooks." />
      </div>
    ...
    <px-modal v-model="modalOpen" :title="$t('Delivery_Details') || 'Delivery Details'" size="lg">
      <div v-if="active" class="pxcfg__deflist">
        <div class="pxcfg__defrow"><span>Event</span><b>{{ active.event }}</b></div>
        <div class="pxcfg__defrow"><span>Webhook</span><b>{{ active.webhook && active.webhook.name }}</b></div>
        <div class="pxcfg__defrow"><span>URL</span><b>{{ active.webhook && active.webhook.url }}</b></div>
        <div class="pxcfg__defrow"><span>Status</span><px-badge :tone="statusTone(active.status)">{{ active.status }}</px-badge></div>
        <div class="pxcfg__defrow"><span>Response code</span><b>{{ active.response_code || '—' }}</b></div>
        <div class="pxcfg__defrow"><span>Attempt</span><b>{{ active.attempt }}</b></div>
        <div class="pxcfg__defrow"><span>Duration</span><b>{{ active.duration_ms }} ms</b></div>
        ...
        <pre class="pxcfg__pre">{{ formatJson(active.payload) }}</pre>
        <h5 class="pxcfg__subhead">Response body</h5>
        <pre class="pxcfg__pre">{{ active.response_body || '—' }}</pre>
      </div>
...
export default {
  ...
  components: { PxPageHeader, PxToolbar, PxTable, PxPagination, PxButton, PxModal, PxBadge, PxEmptyState, VsPx },
  data() {
    return {
      _searchTimer: null,
      isLoading: true,
  ...
  computed: {
    columns() {
      return [
        ...
        { key: "webhook", label: this.$t("Webhook") || "Webhook", sortable: false },
        { key: "status", label: this.$t("Status"), align: "center" },
        { key: "response_code", label: "HTTP", align: "center" },
        { key: "attempt", label: this.$t("Attempt") || "Attempt", align: "center", sortable: true },
