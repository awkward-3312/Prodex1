              ...
              <px-validation-provider :name="$t('Store_URL')" :rules="{ required: true, regex: urlPattern }" v-slot="v">
                <b-form-group :label="$t('Store_URL') + ' *'" class="form-group-modern">
                  <div class="input-icon-wrapper">
                    ...
                    <b-form-input v-model="form.store_url" :state="getState(v)" :placeholder="$t('Enter_Store_URL')" class="form-control-modern" />
                  </div>
...
import { BFormGroup, BFormInput, BFormInvalidFeedback, BAlert, BBadge, BButton, BCard, BCardText, BCol, BRow, BForm } from "@/platform/bootstrap";
import NProgress from 'nprogress';
import moment from 'moment';

export default { components: { BForm, BAlert, BBadge, BButton, BCard, BCardText, BCol, BRow, BFormGroup, BFormInput, BFormInvalidFeedback },
  data() {
    return {
      connecting: false,
      connectionOk: null,
      last_sync_at: null,
      form: { store_url: '', consumer_key: '', consumer_secret: '', wp_username: '', wp_app_password: '' },
      urlPattern: /^https?:\/\//,
    };
  },
  computed: {
    ...
    connectionIcon() { if (this.connectionOk === true) return 'check-circle'; if (this.connectionOk === false) return 'x-circle'; return 'help-circle'; },
  },
  methods: {
    getState(v) { return v.validated ? v.valid : null; },
    loadSettings() {
      return axios.get('woocommerce/settings').then(({ data }) => {
        if (data.settings) { this.form = Object.assign(this.form, data.settings); this.last_sync_at = data.settings.last_sync_at; }
      });
    },
    onSubmit() {
      this.$refs.form.validate().then(valid => {
        if (!valid) { this.toast('danger', this.$t('Please_fill_the_form_correctly')); return; }
        NProgress.start(); NProgress.set(0.1);
        axios.post('woocommerce/settings', this.form).then(() => {
          this.toast('success', this.$t('Successfully_Updated')); NProgress.done(); this.$emit('updated'); this.testConnection();
        }).catch(() => { this.toast('danger', this.$t('InvalidData')); NProgress.done(); });
      });
    },
    testConnection() {
      this.connecting = true;
      axios.post('woocommerce/test-connection').then(({ data }) => {
        this.connectionOk = !!data.ok; this.$emit('connection', this.connectionOk);
...
.form-help-text { display: block; margin-top: 0.5rem; font-size: 12px; }
.connection-status-wrapper { display: flex; align-items: center; }
.connection-status-badge { font-size: 14px; font-weight: 600; border-radius: 20px; box-shadow: 0 2px 8px rgba(0, 0, 0, 0.1); }
.btn-modern-primary { border-radius: 8px; padding: 0.6rem 1.5rem; font-weight: 600; box-shadow: 0 4px 12px rgba(102, 126, 234, 0.3); transition: all 0.3s ease; }
.btn-modern-primary:hover { transform: translateY(-2px); box-shadow: 0 6px 16px rgba(102, 126, 234, 0.4); }
.btn-modern-outline { border-radius: 8px; padding: 0.6rem 1.5rem; font-weight: 600; border-width: 2px; transition: all 0.3s ease; }
...
.guide-intro { font-size: 13px; color: #4a5568; line-height: 1.6; }
.guide-intro code, .guide-list code { background: #e2e8f0; color: #1e293b; padding: 0.2em 0.4em; border-radius: 4px; font-size: 12px; font-family: 'Courier New', monospace; }
.guide-list { list-style: none; padding-left: 0; margin-bottom: 0; }
.guide-list li { padding: 0.5rem 0; display: flex; align-items: flex-start; color: #4a5568; line-height: 1.6; }
.code-block-wrapper { margin-top: 0.75rem; }
.code-label { font-weight: 600; color: #495057; font-size: 13px; margin-bottom: 0.5rem; display: block; }
.code-block { background: #1e293b; color: #e2e8f0; border-radius: 8px; padding: 1rem; white-space: pre-wrap; word-break: break-word; font-size: 13px; line-height: 1.6; margin: 0; border: 1px solid #334155; font-family: 'Courier New', monospace; overflow-x: auto; }
.mini-spinner { display: inline-block; width: 16px; height: 16px; border: 2px solid rgba(102, 126, 234, 0.2); border-top-color: #667eea; border-radius: 50%; animation: spin 0.6s linear infinite; }
