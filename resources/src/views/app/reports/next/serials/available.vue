<template>
  <div class="px-next pxsav">
    <!--
      C3.20 — Informe de números de serie disponibles px-next (solo lectura).
      Ruta real /app/reports/serial_available_report (name serial_available_report).
      Endpoint GET report/serials/available sin cambios. Filtro por almacén,
      búsqueda, orden, paginación y exportación (Excel .xlsx + imprimir).
      Permiso serial_numbers_report. No toca flujos operativos de seriales.
    -->
    <div v-if="!can('serial_numbers_report')" class="pxsav__denied">
    ...
    <template v-else>
      <px-page-header title="Números de serie disponibles" :breadcrumbs="[{ label: 'Informes' }, { label: 'Seriales disponibles' }]">
        <template #actions>
          <vue-excel-xlsx class="pxsav__xlsx" :data="reports" :columns="xlsxColumns" file-name="seriales_disponibles" file-type="xlsx" sheet-name="seriales_disponibles">
      ...
      <div v-if="filtersOpen" class="pxsav__filters">
      ...
      <px-alert v-if="error" tone="danger" title="No se pudo cargar el reporte" class="pxsav__alert">
        {{ error }}
        <template #actions><px-button size="sm" variant="secondary" @click="fetch()">Reintentar</px-button></template>
      </px-alert>

      <div v-if="initialLoading" class="pxsav__pad">
        <px-skeleton variant="table" :rows="10" :columns="6" />
      </div>

      <template v-else>
        <div class="pxsav__tablewrap" :class="{ 'is-busy': refreshing }">
          <px-table
            v-if="rows.length"
            :columns="columns"
            :rows="rows"
          ...
          <px-empty-state v-else icon="scan-barcode" title="Sin seriales disponibles"
            description="No hay números de serie disponibles que coincidan con los filtros." />
        </div>
...
export default {
  name: "SerialAvailableReportNext",
  metaInfo: { title: "Seriales disponibles" },
  components: {
  ...
  computed: {
    ...
    columns() {
      return [
        ...
        { key: "created_at", label: "Fecha de registro", sortable: true, width: "170px" }
      ];
    },
    xlsxColumns() {
...
.pxsav { min-height: 100%; background: var(--pxn-bg); padding: var(--pxn-space-8) var(--pxn-space-9) var(--pxn-space-9); }
@media (max-width: 620px) { .pxsav { padding: var(--pxn-space-6) var(--pxn-space-5); } }
.pxsav__denied { padding: var(--pxn-space-12) 0; }
.pxsav__pad { padding: var(--pxn-space-6) 0; }
.pxsav__alert { margin-top: var(--pxn-space-5); }
.pxsav__filters { margin-top: var(--pxn-space-4); padding: var(--pxn-space-5); border: 1px solid var(--pxn-border); border-radius: var(--pxn-radius-lg); background: var(--pxn-surface); max-width: 420px; }
.pxsav__tablewrap { margin-top: var(--pxn-space-5); transition: opacity var(--pxn-dur-1) var(--pxn-ease); }
.pxsav__tablewrap.is-busy { opacity: 0.55; pointer-events: none; }
.pxsav__xlsx { border: 0; background: none; padding: 0; }
</style>
