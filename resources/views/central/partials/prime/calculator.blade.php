...
@php
    ...
    $fields = [
        ...
        ['dim' => 'max_suppliers',  'label' => __('landing_prime.calc_suppliers'),  'hint' => null,                                       'min' => 0, 'max' => 10000,  'step' => 50,  'groupContinue' => true],
        ['dim' => 'max_products',   'label' => __('landing_prime.calc_products'),   'hint' => __('landing_prime.calc_products_hint'),    'min' => 0, 'max' => 100000, 'step' => 500],
    ];
@endphp

<section id="pricing" class="lp-soft border-t border-slate-100 py-20 sm:py-28 px-5 sm:px-6">
    <div class="max-w-6xl mx-auto">
        <header class="max-w-2xl mx-auto text-center mb-12 lp-reveal">
            <p class="text-xs font-bold uppercase tracking-[0.12em] text-indigo-600 mb-3">{{ __('landing_prime.calc_eyebrow') }}</p>
        ...
        <div id="lpCalc" class="lp-calc grid lg:grid-cols-[1fr_360px] gap-6 lg:gap-8"
            ...
            <div class="lp-card rounded-2xl border border-slate-200 bg-white p-5 sm:p-7 lp-reveal">
                ...
                <div class="space-y-6">
                    @foreach($fields as $f)
                        @if(!empty($f['group']))
                            <p class="text-[13px] font-semibold text-slate-500 pt-1">{{ $f['group'] }}</p>
                        @endif
            ...
            <aside class="lp-calc__summary lp-calc__summary--sticky lp-reveal" data-delay="1">
                <div class="lp-card lp-elevate rounded-2xl border border-slate-200 bg-white p-6">
                    ...
                    <div class="lp-calc__state lp-calc__state--ok">
                        ...
                        <h3 class="mt-3 text-center text-2xl font-bold text-slate-950" data-calc-name>{{ $planForSummary['name'] ?? '—' }}</h3>
                        <div class="mt-2 text-center">
                            ...
                            <span class="text-slate-500 font-medium" data-calc-period>{{ $planForSummary && !$planForSummary['is_free'] ? (($planForSummary['billed_period'] ?? 'month') === 'year' ? __('landing_prime.calc_per_year') : __('landing_prime.calc_per_month')) : '' }}</span>
                        </div>
                        <p class="text-center text-xs text-slate-400 mt-1" data-calc-billnote>{{ $planForSummary && !$planForSummary['is_free'] ? (($planForSummary['billed_period'] ?? 'month') === 'year' ? __('landing_prime.calc_billed_yearly') : __('landing_prime.calc_billed_monthly')) : '' }}</p>

                        <hr class="my-5 border-slate-100">
                        <p class="text-xs font-bold uppercase tracking-wide text-slate-500 mb-3">{{ __('landing_prime.calc_includes') }}</p>
                    ...
                    <div class="lp-calc__state lp-calc__state--custom text-center">
                        <p><span class="lp-calc__badge">{{ __('landing_prime.calc_custom_title') }}</span></p>
                        <p class="mt-3 text-sm text-slate-600">{{ __('landing_prime.calc_custom_lead') }}</p>
                        ...
                        <a class="lp-btn lp-btn--primary w-full mt-4" data-calc-sales href="{{ $salesHref }}" @if($salesExternal) target="_blank" rel="noopener noreferrer" @endif>
                            {{ __('landing_prime.calc_custom_cta') }}
                        </a>
                    </div>

                    {{-- Estado insufficient_plan_data --}}
                    <div class="lp-calc__state lp-calc__state--nodata text-center">
                        <h3 class="text-lg font-bold text-slate-950">{{ __('landing_prime.calc_nodata_title') }}</h3>
                        <p class="mt-2 text-sm text-slate-600">{{ __('landing_prime.calc_nodata_lead') }}</p>
                        <a class="lp-btn lp-btn--primary w-full mt-4" data-calc-sales href="{{ $salesHref }}" @if($salesExternal) target="_blank" rel="noopener noreferrer" @endif>
            ...
            <div id="lpCalcPlans" class="lp-calc__plans lg:col-span-2 lp-reveal">
                @if(!empty($calcPlans))
                    <p class="text-xs font-bold uppercase tracking-wide text-slate-500 mb-4 mt-2">{{ __('landing_prime.calc_plans_title') }}</p>
                        ...
                        @foreach($calcPlans as $p)
                            <div class="lp-card lp-card--hover rounded-2xl border border-slate-200 bg-white p-5 flex flex-col">
                                ...
                                <p class="mt-1 mb-4">
                                    ...
                                    @else
                                        <span class="text-2xl font-extrabold text-slate-950 lp-tnum">{{ $fmtMoney($p['price_monthly']) }}</span>
                                        <span class="text-sm text-slate-500">{{ __('landing_prime.calc_per_month') }}</span>
                                    @endif
                                </p>
                                @if($p['yearly_available'])
                                    <p class="text-xs text-slate-400 -mt-3 mb-3">{{ __('landing_prime.calc_plan_from') }} {{ $fmtMoney($p['price_yearly']) }} {{ __('landing_prime.calc_per_year') }} · &minus;{{ $p['yearly_savings_percent'] }}%</p>
                                @endif
